...
//		}
	...
	public int GetValIntByKey(String key)
	{
		...
		} catch (RuntimeException ex)
		{
			...
			if (!SystemConfig.getIsDebug())
			{
				throw new RuntimeException("@cls="+this.toString()+"@字段["
	...
	public String getPK()
	{
		...
		for (Attr attr : this.getEnMap().getAttrs())
		{
			/*if (attr.getKey().equals("No"))
			{
				return "No";
			}
			else if (attr.getKey().equals("OID"))
			{
				return "OID";
			}
			else if (attr.getKey().equals("MyPK"))
			{
				return "MyPK";
			}*/
			if (attr.getMyFieldType() == FieldType.PK
