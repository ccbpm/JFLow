...
public abstract class Entity extends EnObj
{
		...
		try
		{
			...
			switch (this.getEnMap().getEnDBUrl().getDBType())
			{
				...
				case Oracle:
					...
					break;
				case MySQL:
					sql = "SELECT CONVERT(MAX(CAST(" + field 
							+ " AS SIGNED INTEGER)),SIGNED) +1 AS No FROM " 
							+ this.get_enMap().getPhysicsTable();
					break;
				case Informix:
					sql = "SELECT MAX(" + field + ") +1 AS No FROM "
	...
	public final boolean IsExit(String pk, Object val) throws Exception
	{
		...
		// else
		// {
		// string sql = "SELECT " + pk + " FROM " + this.getEnMap().getPhysicsTable() +
		// " WHERE " + pk + " ='" + val + "'";
	...
	private boolean CheckDB()
	{
		...
		for (EnDtl dtl : this.getEnMap().getDtls())
		{
			...
			//
			// //string
			// sql="SELECT "+dtl.RefKey+" FROM  "+dtl.Ens.GetNewEntity.getEnMap().getPhysicsTable()+"   WHERE  "+dtl.RefKey+" ='"+this.PKVal.ToString()
			// +"' ";
			// DataTable dt= DBAccess.RunSQLReturnTable(sql);
			// if(dt.Rows.Count==0)
			// continue;
...
//						this);
	...
	public final void SaveFileToDB(String saveToField, String filefullName) throws Exception
	{
		...
		} catch (RuntimeException ex)
		{
			...
			{
				sql = "ALTER TABLE " + this.getEnMap().getPhysicsTable()
						+ " ADD " + saveToField + " Blob NULL ";
			}
			
			if (BP.DA.DBAccess.getAppCenterDBType().equals(DBType.MySQL))
			{
				sql = "ALTER TABLE " + this.getEnMap().getPhysicsTable()
						+ " ADD " + saveToField + " MediumBlob NULL ";
			}
	...
	private void CheckPhysicsTable_Informix() throws Exception
	{
		...
		for (Attr attr : attrs)
		{
			...
			sql = "SELECT DATA_TYPE FROM ALL_TAB_COLUMNS WHERE OWNER = '"+SystemConfig.getUser().toUpperCase()+"' AND TABLE_NAME='"
			...
			if (val.indexOf("CHAR") != -1)
			{
				...
				//由于ALL_TAB_COLUMNS表中有可能会出现用户名(owner)不一样，表名(table_name)一样的数据，导至会去修改其它用户下的表
				//增加查询条件owner = 当前系统配置的连接用户(SystemConfig.getUser().toUpperCase())	
				sql = "SELECT OWNER FROM ALL_TAB_COLUMNS WHERE OWNER = '"+SystemConfig.getUser().toUpperCase()+"' AND  upper(TABLE_NAME)='"
	...
	private void CheckPhysicsTable_Ora() throws Exception
	{
		...
		for (Attr attr : this.getEnMap().getAttrs())
		{
			...
			//由于ALL_TAB_COLUMNS表中有可能会出现用户名(owner)不一样，表名(table_name)一样的数据，导至会去修改其它用户下的表
			//增加查询条件owner = 当前系统配置的连接用户(SystemConfig.getUser().toUpperCase())	
			sql = "SELECT DATA_LENGTH AS LEN, OWNER FROM ALL_TAB_COLUMNS WHERE OWNER = '"+SystemConfig.getUser().toUpperCase()+"' AND upper(TABLE_NAME)='"
					+ this.getEnMap().getPhysicsTableExt().toUpperCase()
					+ "' AND UPPER(COLUMN_NAME)='"
					+ attr.getField().toUpperCase()
					+ "' AND DATA_LENGTH < "
		...
		for (Attr attr : attrs)
		{
			...
			sql = "SELECT DATA_TYPE FROM ALL_TAB_COLUMNS WHERE OWNER = '"+SystemConfig.getUser().toUpperCase()+"' AND upper(TABLE_NAME)='"
					+ this.getEnMap().getPhysicsTableExt().toUpperCase()
					+ "' AND UPPER(COLUMN_NAME)='"
					+ attr.getField().toUpperCase() + "' ";
			String val = DBAccess.RunSQLReturnString(sql);
			...
			if (val.indexOf("CHAR") != -1)
			{
				...
				//由于ALL_TAB_COLUMNS表中有可能会出现用户名(owner)不一样，表名(table_name)一样的数据，导至会去修改其它用户下的表
				//增加查询条件owner = 当前系统配置的连接用户(SystemConfig.getUser().toUpperCase())
				sql = "SELECT A.OWNER FROM ALL_TAB_COLUMNS WHERE OWNER = '"+SystemConfig.getUser().toUpperCase()+"' AND upper(TABLE_NAME)='"
						+ this.getEnMap().getPhysicsTableExt().toUpperCase()
						+ "' AND UPPER(COLUMN_NAME)='"
						+ attr.getField().toUpperCase() + "' ";
				String OWNER = DBAccess.RunSQLReturnString(sql);
