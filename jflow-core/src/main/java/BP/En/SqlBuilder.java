...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	*/
	...
	public static String RetrieveForPara_bak(Entity en)
	{
		switch (en.getEnMap().getEnDBUrl().getDBType())
		{
			...
			case MySQL:
			case Access:
				if (en.getEnMap().getHisFKAttrs().Count == 0)
				{
	...
	public static String GenerFormOfOra(Entity en)
	{
		...
		if (en.getEnMap().getHisFKEnumAttrs().Count == 0)
		{
	...
	*/
	public static String GenerCreateTableSQLOfMS(Entity en)
	{
		...
		if (attrs.Count == 0)
		{
			throw new RuntimeException("@" + en.getEnDesc() + " , [" + en.getEnMap().getPhysicsTable() + "]没有属性/字段集合 attrs.Count = 0 ,能执行数据表的创建.");
		}
			...
			switch (attr.getMyDataType())
			{
				...
				case DataType.AppInt:
					if (attr.getIsPK() && attr.getKey().equals("OID"))
					{
						if (attr.getUIBindKey().equals("1"))
						{
							sql += attr.getField() + " INT(4) primary key not null auto_increment COMMENT '" + attr.getDesc() + "',";
						}
						else
						{
							sql += attr.getField() + " INT NOT NULL COMMENT '" + attr.getDesc() + "',";
						}
					}
					else
					{
		...
		for (Attr attr : en.getEnMap().getAttrs())
		{
			...
			switch (attr.getMyDataType())
			{
				case DataType.AppString:
					...
					if (attr.getDefaultVal() == null || tempVar instanceof String ? (String)tempVar : null.equals(""))
					{
				...
				case DataType.AppInt:
					...
					if (attr.getMyFieldType() == FieldType.Enum || attr.getMyFieldType() == FieldType.PKEnum)
					{
						...
						Sys.SysEnums ses = new BP.Sys.SysEnums(attr.getUIBindKey(), attr.UITag);
	...
	*/
	public static String SelectSQLOfMS(Entity en, int topNum)
	{
		...
		if (en.getEnMap().getAttrs().Count == 0)
		{
...
///#warning 2011-12-03 不应出现异常。
	...
	*/
	...
	public static String UpdateOfMSAccess(Entity en, String[] keys)
	{
		...
		sql = "UPDATE " + en.getEnMap().getPhysicsTable() + " SET " + val.substring(1) +
	...
	*/
				...
				case DataType.AppString:
					...
					else
					{
						if (attr.getUIIsDoc() && attr.getKey().equals("Doc"))
						{
							...
							if (doc.length() >= 2000)
							{
								Sys.SysDocFile.SetValV2(en.toString(), en.getPKVal().toString(), doc);
		...
		for (Attr attr : map.getAttrs())
		{
			...
			switch (attr.getMyDataType())
			{
				...
				case DataType.AppMoney:
					...
					{
						ps.Add(attr.getKey(), BigDecimal.Parse(str));
					}
					break;
				case DataType.AppDate: // 如果是日期类型。
				case DataType.AppDateTime:
	...
	public static Paras GenerParas(Entity en, String[] keys)
	{
			...
			for (Attr attr : map.getAttrs())
			{
				...
				switch (attr.getMyDataType())
				{
					...
					case DataType.AppInt:
						...
						{
							if (en.getRow().get(attr.getKey()) == DBNull.Value)
							{
								ps.Add(attr.getKey(), Integer.parseInt(attr.getDefaultValOfReal()));
								continue;
							}

							String strInt = en.getRow().get(attr.getKey()).toString();
					...
					case DataType.AppDouble:
						...
						if (DataType.IsNullOrEmpty(str))
						{
							if (IsEnableNull)
							{
								ps.Add(attr.getKey(), DBNull.Value);
							}
							else
							{
								ps.Add(attr.getKey(), 0);
					...
					case DataType.AppMoney:
						...
						ps.Add(attr.getKey(), Double.parseDouble(str, System.Globalization.NumberStyles.Any));
		...
		catch (RuntimeException ex)
		{
			...
			errKey = "@attrKey=" + attr.getKey() + ",AttrVal=" + en.getRow().get(attr.getKey()) + ",DataType=" + attr.getMyDataTypeStr();
	...
	public static String UpdateForPara(Entity en, String[] keys)
	{
		...
		switch (en.getEnMap().getEnDBUrl().getDBType())
		{
			...
			case MySQL:
			case PostgreSQL:
				sql = "UPDATE " + en.getEnMap().getPhysicsTable() + " SET " + val +
					" WHERE " + SqlBuilder.GenerWhereByPK(en, "@");
				break;
			case Informix:
				sql = "UPDATE " + en.getEnMap().getPhysicsTable() + " SET " + val +
			...
			switch (attr.getMyDataType())
			{
				case DataType.AppString:
					...
					if (attr.getUIIsDoc() && attr.getKey().equals("Doc"))
					{
						...
						if (str.length() >= 2000)
						{
							Sys.SysDocFile.SetValV2(en.toString(), en.getPKVal().toString(), str);
							if (attrs.Contains("DocLength"))
							{
								en.SetValByKey("DocLength", str.length());
							}
