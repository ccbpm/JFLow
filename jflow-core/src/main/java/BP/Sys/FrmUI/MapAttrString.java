	...
	*/
	...
	public Map getEnMap()
	{
		...
		map.SetHelperAlert(MapAttrAttr.ColSpan, "对于傻瓜表单有效: 标识该字段TextBox横跨的宽度,占的单元格数量.");
			...
			//文本占单元格数量
		map.AddDDLSysEnum(MapAttrAttr.TextColSpan, 1, "Label单元格数量", true, true, "ColSpanAttrString", "@1=跨1个单元格@2=跨2个单元格@3=跨3个单元格@4=跨4个单元格@5=跨5个单元格@6=跨6个单元格");
...
//		map.AddRefMethod(rm);
	...
	*/
	 ...
	 * @throws Exception
	 */
	public String DoSetFlowBBS() throws Exception {
		...
		if (mapAttrs.size() == 0) {
			...
			this.setUIVisible(false);
			this.Update();
			return "设置成功,当前文本框已经是评论组件了,请关闭掉当前的窗口,并关闭当前的设计器,重新登录.";
		}
