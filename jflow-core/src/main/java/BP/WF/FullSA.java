...
public class FullSA {
	...
	public FullSA(WorkNode currWorkNode) throws Exception {
		...
		for (Node item : nds.ToJavaList()) {
			...
			if (item.getHisDeliveryWay() == DeliveryWay.BySpecNodeEmp
					&& String.valueOf(currND.getNodeID()).equals(item.getDeliveryParas())) {
			...
			if (item.getHisDeliveryWay() == DeliveryWay.ByStation
				...
				case MSSQL:
					...
					sql += " on a.FK_Station=b.FK_Station where FK_Node=" + dbStr
							+ "FK_Node) as y on x.No=y.FK_Emp inner join Port_DeptEmp z on";
					sql += " x.No=z.FK_Emp where z.FK_Dept =" + dbStr + "FK_Dept order by x.No";
					break;
				default:
					sql = "SELECT No FROM Port_Emp WHERE NO IN " + "(SELECT  FK_Emp  FROM " + BP.WF.Glo.getEmpStation()
	...
	private void InitDT(SelectAccper sa, Node nd) throws Exception {
		...
		if (this.LastTimeDot == null) {
			Paras ps = new Paras();
			ps.SQL = "SELECT SDT FROM WF_GenerWorkerlist WHERE WorkID=" + ps.getDBStr() + "WorkID AND FK_Node="
					+ ps.getDBStr() + "FK_Node";
		...
		dtOfShould = Glo.AddDayHoursSpan(this.LastTimeDot, nd.getTimeLimit(), nd.getTimeLimitHH(), nd.getTimeLimitMM(),
				nd.getTWay());
