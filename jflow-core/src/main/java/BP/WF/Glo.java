...
public class Glo {
	...
	*/
	...
	public static void GenerWord(Object filename, Work wk) {
		 ...
		 * 
		 * GroupField currGF = new GroupField(); MapAttrs mattrs = new
		 * MapAttrs(enName); GroupFields gfs = new GroupFields(enName); MapDtls
		 * dtls = new MapDtls(enName); for (MapDtl dtl : dtls.ToJavaList()) {
		 * dtl.IsUse = false; }
		 * 
		 * // 计算出来单元格的行数。 int rowNum = 0; for (GroupField gf : gfs) { rowNum++;
		 * boolean isLeft = true; for (MapAttr attr : mattrs) { if
		 * (attr.UIVisible == false) { continue; }
		 * 
		 * if (attr.GroupID != gf.OID) { continue; }
		 * 
		 * if (attr.UIIsLine) { rowNum++; isLeft = true; continue; }
		 * 
		 * if (isLeft == false) { rowNum++; } isLeft = !isLeft; } }
		 * 
		 * rowNum = rowNum + 2 + dtls.size();
		 * 
		 * // 创建Word文档 String CheckedInfo = ""; String message = ""; Object
		 * Nothing = System.Reflection.Missing.getValue();
		 */
