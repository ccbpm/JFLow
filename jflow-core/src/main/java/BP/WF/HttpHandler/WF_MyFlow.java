...
public class WF_MyFlow extends WebContralBase {
	...
	public String InitToolBar(){
		...
		String tKey = "xxxxx"; // new java.util.Date().ToString("yyyy-MM-dd - hh:mm:ss");
		String toolbar = "";
		try
		{
			if (this.getIsCC())
...
//                         
			...
			if (btnLab.getWorkCheckEnable() && isAskFor == false)
			{
				...
				toolbar += "<input type=button  value='" + btnLab.getWorkCheckLab() + "' enable=true onclick=\"WinOpen('" + urlr1 + "','dsdd'); \" />";
			}
			...
			{
				if (HisGenerWorkFlow.getParas_Focus() == true)
				{
			...
			{
				if (HisGenerWorkFlow.getParas_Confirm() == true)
				{
