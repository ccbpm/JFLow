...
//
//				if (currWn.getHisNode().getHisRunModel() == RunModel.SubThread)
//				{
//					ps.Add("OID", currWn.getHisWork().getFID());
//				}
//				else
//				{
//					ps.Add("OID", this.WorkID);
//				}
		...
		if (town.getHisNode().getHisDeliveryWay() == DeliveryWay.BySpecNodeEmpStation)
		{
			...
			if (DotNetToJavaStringHelper.isNullOrEmpty(para) ==false)
			{
			  ...
			  //  throw new Exception("@流程设计错误，到达的节点（" + town.getHisNode().Name + "）在指定的节点中没有数据，无法找到工作的人员，指定的节点是:"+para);
			}
			else
			{
		...
		if (this.currWn.getHisNode().getGroupStaNDs() != town.getHisNode().getGroupStaNDs())
		{
			...
			if (flowAppType == FlowAppType.PRJ)
			{
				...
				if (dt.Rows.size() == 0)
				{
					...
					if (BP.WF.Glo.getOSModel() == OSModel.OneOne)
					{
						sql = "SELECT No FROM Port_Emp WHERE NO IN " + "(SELECT  FK_Emp  FROM " + BP.WF.Glo.getEmpStation() + " WHERE FK_Station IN (SELECT FK_Station FROM WF_NodeStation WHERE FK_Node=" + dbStr + "FK_Node))" + " AND  NO IN " + "(SELECT No as FK_Emp FROM Port_Emp WHERE FK_Dept =" + dbStr + "FK_Dept)";
						sql += " ORDER BY No ";
					}
					else
					{
...
//ORIGINAL LINE: case "SpecStations":
					if (specName.equals("SpecStations")) //按岗位
					{
							...
							for (String station : stations)
							{
								...
								{
									//排除为空编号
									if (empRow.getValue(0) == null || DotNetToJavaStringHelper.isNullOrEmpty(empRow.getValue(0).toString()))
									{
										continue;
									}
