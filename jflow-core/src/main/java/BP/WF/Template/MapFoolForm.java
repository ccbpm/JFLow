...
public class MapFoolForm extends EntityNoName
{
	/** 
	 是否是节点表单?
	*/
	public final boolean getIsNodeFrm()
	{
		...
		if ("ND".equals(getNo().substring(0, 2)))
		{
			return true;
		}
...
//		return i;
	...
	*/
	...
	public Map getEnMap()
	{
		...
		rm.Icon = SystemConfig.getCCFlowWebPath() + "WF/Admin/CCBPMDesigner/Img/field.png";
		rm.Visable = true;
		rm.refMethodType = RefMethodType.RightFrameOpen;
		rm.Target = "_blank";
		//map.AddRefMethod(rm);
	...
	*/
	public final String DoNodeFrmCompent()
	{
		...
		{
			return SystemConfig.getCCFlowWebPath() + "WF/Comm/RefFunc/UIEn.jsp?EnName=BP.WF.Template.FrmNodeComponent&PK=" + this.getNo().replace("ND", "") + "&t=" + DataType.getCurrentDataTime();
		}
		else
		{
			return SystemConfig.getCCFlowWebPath() + "WF/Admin/FoolFormDesigner/Do.jsp&DoType=FWCShowError";
		}
	}
	/** 
	 执行旧版本的兼容性检查.
	*/
	public final String DoCheckFixFrmForUpdateVer()
	{
		...
		String str = "";
		 ...
		 // 处理失去分组的字段. 
		String sql = "SELECT MyPK FROM Sys_MapAttr WHERE FK_MapData='" + this.getNo() + "' AND GroupID NOT IN (SELECT OID FROM Sys_GroupField WHERE EnName='" + this.getNo() + "' AND (CtrlType IS NULL OR CtrlType='') ) ";
		...
		for (MapDtl dtl : dtls.ToJavaList())
		{
			...
			if (gf.IsExit(GroupFieldAttr.CtrlID, dtl.getNo()) == true && !StringHelper.isNullOrEmpty(gf.getCtrlType()))
			{
				continue;
			}
		...
		{
			GroupField gf = new GroupField();
			if (gf.IsExit(GroupFieldAttr.CtrlID, fram.getMyPK()) == true && !StringHelper.isNullOrEmpty(gf.getCtrlType()))
			{
				continue;
			}
		...
		{
			GroupField gf = new GroupField();
			if (gf.IsExit(GroupFieldAttr.CtrlID, ath.getMyPK()) == true && !StringHelper.isNullOrEmpty(gf.getCtrlType()))
			{
				continue;
			}
	...
	*/
	...
	{
		return SystemConfig.getCCFlowWebPath() + "WF/Admin/FoolFormDesigner/MapExt/RegularExpressionBatch.jsp?FK_Flow=&FK_MapData=" + this.getNo() + "&t=" + DataType.getCurrentDataTime();
	}
	/** 
	 批量修改字段
	...
	*/
	...
	{
		return SystemConfig.getCCFlowWebPath() + "WF/Admin/FoolFormDesigner/BatchEdit.htm?FK_MapData=" + this.getNo() + "&t=" + DataType.getCurrentDataTime();
	}
	/** 
	 排序字段顺序
	...
	*/
	...
	{
		return SystemConfig.getCCFlowWebPath() + "WF/Admin/AttrNode/SortingMapAttrs.htm?FK_Flow=&FK_MapData=" + this.getNo() + "&t=" + DataType.getCurrentDataTime();
	}
	/** 
	 设计表单
	...
	*/
	...
	{
		String url = SystemConfig.getCCFlowWebPath() + "WF/Admin/FoolFormDesigner/CCForm/Frm.jsp?FK_MapData=" + this.getNo() + "&UserNo=" + BP.Web.WebUser.getNo() + "&SID=" + WebUser.getSID() + "&AppCenterDBType=" + BP.DA.DBAccess.getAppCenterDBType() + "&CustomerNo=" + BP.Sys.SystemConfig.getCustomerNo();
		try {
			PubClass.WinOpen(null, url, 800, 650);
		} catch (IOException e) {
			Log.DebugWriteError("设计表单 DoDFrom():"+e.getMessage());
		}
		return null;
	}
	/** 
	...
	*/
	...
	{
		String url = SystemConfig.getCCFlowWebPath() + "WF/Admin/FoolFormDesigner/Designer.htm?FK_MapData=" + this.getNo() + "&UserNo=" + BP.Web.WebUser.getNo() + "&SID=" + WebUser.getSID() + "&AppCenterDBType=" + BP.DA.DBAccess.getAppCenterDBType() + "&CustomerNo=" + BP.Sys.SystemConfig.getCustomerNo();
		try {
			PubClass.WinOpen(null, url, 800, 650);
		} catch (IOException e) {
			Log.DebugWriteError("设计傻瓜表单 DoDFromCol4():"+e.getMessage());
		}
		return null;
	}
	/** 
	...
	*/
	...
	{
		return SystemConfig.getCCFlowWebPath() + "WF/Admin/FoolFormDesigner/MapExt/WordFrm.jsp?s=34&FK_MapData=" + this.getNo() + "&ExtType=WordFrm&RefNo=";
	}
	public final String DoExcelFrm()
	{
		return SystemConfig.getCCFlowWebPath() + "WF/Admin/FoolFormDesigner/MapExt/ExcelFrm.jsp?s=34&FK_MapData=" + this.getNo() + "&ExtType=ExcelFrm&RefNo=";
	}
	public final String DoPageLoadFull()
	{
		return SystemConfig.getCCFlowWebPath() + "WF/Admin/FoolFormDesigner/MapExt/PageLoadFull.jsp?s=34&FK_MapData=" + this.getNo() + "&ExtType=PageLoadFull&RefNo=";
	}
	public final String DoInitScript()
	{
		return SystemConfig.getCCFlowWebPath() + "WF/Admin/FoolFormDesigner/MapExt/InitScript.jsp?s=34&FK_MapData=" + this.getNo() + "&ExtType=PageLoadFull&RefNo=";
	}
	/** 
	 表单属性.
	...
	*/
	...
	{
		return SystemConfig.getCCFlowWebPath() + "WF/Admin/FoolFormDesigner/MapExt/BodyAttr.jsp?s=34&FK_MapData=" + this.getNo() + "&ExtType=BodyAttr&RefNo=";
	}
	/** 
	 表单事件
	...
	*/
	...
	{
		return SystemConfig.getCCFlowWebPath() + "WF/Admin/AttrNode/Action.jsp?FK_MapData=" + this.getNo() + "&T=sd&FK_Node=0";
	}
	/** 
	 导出
	...
	*/
	...
	{
		return SystemConfig.getCCFlowWebPath() + "WF/Admin/FoolFormDesigner/MapExt/List.jsp?FK_MapData=" + this.getNo() + "&T=sd";
	}
	/** 
	 导出表单
	...
	*/
	...
	{
		String urlExt = SystemConfig.getCCFlowWebPath() + "WF/Admin/XAP/DoPort.jsp?DoType=DownFormTemplete&FK_MapData=" + this.getNo();
		try {
			PubClass.WinOpen(null, urlExt, 900, 1000);
		} catch (IOException e) {
			Log.DebugWriteError("导出表单 DoExp() :"+e.getMessage());
		}
		return null;
	}
	//#endregion 方法.
