...
public class MapFrmFree extends EntityNoName {
	...
	public final String DoDesignerFool() {
		return Glo.getCCFlowAppPath() + "WF/Admin/FoolFormDesigner/Designer.htm?FK_MapData=" + this.getNo() + "&MyPK="
	...
	public final String DoCheckFixFrmForUpdateVer() throws Exception {
		...
		// 处理失去分组的字段.
		String sql = "SELECT MyPK FROM Sys_MapAttr WHERE FK_MapData='" + this.getNo()
				+ "' AND GroupID NOT IN (SELECT OID FROM Sys_GroupField WHERE EnName='" + this.getNo()
				+ "' AND CtrlType='' ) ";
		...
		for (MapFrame fram : frams.ToJavaList()) {
			GroupField gf = new GroupField();
			if (gf.IsExit(GroupFieldAttr.CtrlID, fram.getMyPK()) == true
					&& !DotNetToJavaStringHelper.isNullOrEmpty(gf.getCtrlType())) {
				continue;
			}
		...
		for (FrmAttachment ath : aths.ToJavaList()) {
			GroupField gf = new GroupField();
			if (gf.IsExit(GroupFieldAttr.CtrlID, ath.getMyPK()) == true
					&& !DotNetToJavaStringHelper.isNullOrEmpty(gf.getCtrlType())) {
				continue;
			}
