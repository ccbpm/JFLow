...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	{
		String str = "";
		for (GenerWorkerList wl : this.HisWorkerLists)
		{
			str = wl.getFK_Emp() + ",";
		}
		return str;
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	*/
	...
	{
		String tempFile = SystemConfig.PathOfTemp + "\\" + this.getWorkID() + ".doc";
	...
	*/
	public final void DeleteToNodesData(Nodes nds)
	{
		...
		for (Node nd : nds)
		{
			...
			if (this.getHisFlow().getPTable().equals(wk.EnMap.PhysicsTable))
			{
				continue;
			}
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		if (town.getHisNode().getHisCHWay() == CHWay.ByTime)
		{
			...
			{
				dtOfShould = LocalDateTime.parse(sdtOdNode);
			}
			else
			{
				//按天、小时考核
				if (town.getHisNode().GetParaInt("CHWayOfTimeRole") == 0)
				{
					...
					dtOfShould = Glo.AddDayHoursSpan(LocalDateTime.now(), timeLimit, this.town.getHisNode().getTimeLimitHH(), this.town.getHisNode().getTimeLimitMM(), this.town.getHisNode().getTWay());
				}
				//按照节点字段设置
		...
		//求警告日期.
		LocalDateTime dtOfWarning = LocalDateTime.now();
		...
		else
		{
			//计算警告日期.
			//增加小时数. 考虑到了节假日.
			dtOfWarning = Glo.AddDayHoursSpan(LocalDateTime.now(), this.town.getHisNode().getWarningDay(), 0, 0, this.town.getHisNode().getTWay());
		}
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		if (dt.Rows.size() == 1)
		{
			...
			wl.setFK_Emp(dt.Rows[0][0].toString());
			...
			wl.setFK_EmpText(emp.Name);
			wl.setFK_Dept(emp.FK_Dept);
			wl.setFK_DeptT(emp.FK_DeptText);
			...
			{
				wl.setGroupMark(dt.Rows[0][2].toString()); //第3个列是分组标记.
			...
			{
				wl.setSender(GuestUser.No + "," + GuestUser.Name);
				wl.setGuestNo(GuestUser.No);
				wl.setGuestName(GuestUser.Name);
			}
			else
			{
				wl.setSender(WebUser.No + "," + WebUser.Name);
			}
			...
			if (town.getHisNode().getIsGuestNode())
			{
				...
				else
				{
					...
					{
						Object tempVar = town.getHisNode().getDeliveryParas().Clone();
						...
						wl.setGuestNo(mydt.Rows[0][0].toString());
						wl.setGuestName(mydt.Rows[0][1].toString());
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		if (at == ActionType.SubThreadForward)
		{
			...
			for (GenerWorkerList wl : this.HisWorkerLists)
			{
				this.AddToTrack(at, wl, BP.WF.Glo.multilingual("子线程", "WorkNode", "sub_thread", new String[0]), this.town.getHisWork().getOID());
			}
			//写入到日志.
		}
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		{
			for (GenerWorkerList gwl : this.HisWorkerLists)
			{
				ids += gwl.getFK_Emp() + ",";
				names += gwl.getFK_EmpText() + ",";
			}
		...
		if (this.getHisNode().getSubFlowYanXuNum() > 0)
		{
			...
			if (ygflows.size() != 0 && 1 == 2)
			{
				for (SubFlowYanXu item : ygflows)
				{
					...
					{
						isPass = BP.WF.Glo.CondExpPara(item.getCondExp(), this.rptGe.Row, this.getWorkID());
					}

					if (item.getExpType() == ConnDataFrom.SQL)
					{
						isPass = BP.WF.Glo.CondExpSQL(item.getCondExp(), this.rptGe.Row, this.getWorkID());
					}
		...
		while (true)
		{
			...
			if (mynd.getIsEndNode())
			{
				...
				if (this.getHisGenerWorkFlow().getTransferCustomType() == TransferCustomType.ByCCBPMDefine)
				{
					...
					String msg = this.getHisWorkFlow().DoFlowOver(ActionType.FlowOver, "流程已经走到最后一个节点，流程成功结束。", mynd, this.rptGe);
			...
			if (!nd.getNodeFrmID().equals(mywork.NodeFrmID))
			{
				...
				if (skipWork.EnMap.PhysicsTable != this.rptGe.getEnMap().PhysicsTable)
				{
						...
						if (athDesc.size() > 0)
						{
							...
							if (athDBs.size() > 0)
							{
								...
								for (FrmAttachmentDB athDB : athDBs)
								{
									...
									athDB_N.Copy(athDB);
									athDB_N.FK_MapData = "ND" + nd.getNodeID();
									athDB_N.RefPKVal = String.valueOf(this.getWorkID());
									...
									athDB_N.FK_FrmAttachment = athDB_N.FK_FrmAttachment.Replace("ND" + this.getHisNode().getNodeID(), "ND" + nd.getNodeID());
						...
						if (this.getHisNode().getMapData().FrmImgAths.size() > 0)
						{
							...
							if (athDBs.size() > 0)
							{
								...
								for (FrmImgAthDB athDB : athDBs)
								{
									...
									athDB_N.FK_MapData = "ND" + nd.getNodeID();
									athDB_N.RefPKVal = String.valueOf(this.getWorkID());
									athDB_N.MyPK = this.getWorkID() + "_" + idx + "_" + athDB_N.FK_MapData;
									athDB_N.FK_FrmImgAth = athDB_N.FK_FrmImgAth.Replace("ND" + this.getHisNode().getNodeID(), "ND" + nd.getNodeID());
									athDB_N.Save();
								}
							}
						}
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
						...
						if (this.getHisNode().getMapData().FrmEles.size() > 0)
						{
							...
							if (eleDBs.size() > 0)
							{
								...
								/*说明当前节点有附件数据*/
								for (FrmEleDB eleDB : eleDBs)
								{
									FrmEleDB eleDB_N = new FrmEleDB();
									eleDB_N.Copy(eleDB);
									eleDB_N.FK_MapData = "ND" + nd.getNodeID();
									eleDB_N.GenerPKVal();
									eleDB_N.Save();
								}
							}
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
						...
						if (dtls.size() > 0)
						{
							...
							for (Sys.MapDtl dtl : dtls)
							{
								...
								{
									if (todtl.Name.substring(6, todtl.Name.Length).equals(dtl.Name.substring(6, dtl.Name.Length)))
									{
								...
								DBAccess.RunSQL("DELETE FROM " + toDtl.PTable + " WHERE RefPK=" + dbStr + "RefPK", "RefPK", String.valueOf(this.getWorkID()));
								...
								for (GEDtl gedtl : gedtls)
								{
									...
									dtCopy.RefPK = String.valueOf(this.getWorkID());
									...
									if (toDtl.IsEnableAthM)
									{
										...
										if (athDBs.size() > 0)
										{
											...
											for (FrmAttachmentDB athDB : athDBs)
											{
												...
												athDB_N.MyPK = toDtl.No + "_" + dtCopy.OID + "_" + String.valueOf(i);
												athDB_N.FK_FrmAttachment = athDB_N.FK_FrmAttachment.Replace("ND" + this.getHisNode().getNodeID(), "ND" + nd.getNodeID());
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
								...
								if (isEnablePass)
								{
									...
									String startUser = "SELECT Rec FROM " + md.PTable + " WHERE OID=" + this.getWorkID();
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
			...
			if (dt == null || dt.Rows.size() == 0)
			{
				...
				{
					this.AddToTrack(ActionType.Skip, this.getExecer(), this.getExecerName(), nd.getNodeID(), nd.getName(), BP.WF.Glo.multilingual("自动跳转(当没有找到处理人时)", "WorkNode", "system_error_jump_automatically_1", new String[0]), getndFrom());
					setndFrom(nd);
					continue;
				}
				else
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
				...
				{
					/*如果发现了，当前人员包含处理人集合. */
					this.AddToTrack(ActionType.Skip, Executor, ExecutorName, nd.getNodeID(), nd.getName(), BP.WF.Glo.multilingual("自动跳转(当没有找到处理人时)", "WorkNode", "system_error_jump_automatically_2", new String[0]), getndFrom());
			...
			if (nd.getAutoJumpRole1())
			{
				...
				{
					// 如果出现了处理人就是提交人的情况.
					String sql = "SELECT COUNT(*) FROM WF_GenerWorkerList WHERE FK_Emp='" + dr.get(0).toString() + "' AND WorkID=" + this.getWorkID();
			...
			if (nd.getAutoJumpRole2())
			{
				...
				for (DataRow dr : dt.Rows)
				{
					String sql = "SELECT COUNT(*) FROM WF_GenerWorkerList WHERE FK_Emp='" + dr.get(0) +
				...
				if (isHave == true)
				{
					...
					Object tempVar = nd.getMapData().MapExts.GetEntityByKey(MapExtAttr.ExtType, MapExtXmlList.PageLoadFull);
					MapExt item = tempVar instanceof MapExt ? (MapExt)tempVar : null;
					BP.WF.Glo.DealPageLoadFull(wk, item, nd.getMapData().MapAttrs, nd.getMapData().MapDtls);
					...
					for (FrmImg img : imgs)
					{
						...
						if (!DataType.IsNullOrEmpty(img.Tag0) && img.Tag0.Contains("^") && img.Tag0.split("[^]", -1).Length == 4)
						{
							...
							if (fk_dept.equals("all"))
							{
								...
								{
									//判断字段是否存在
									for (MapAttr attr : nd.getMapData().MapAttrs)
									{
						...
						try
						{
							...
							myPK = myPK + "_" + wk.getOID() + "_" + img.MyPK;
							...
							if (imgDb != null && !DataType.IsNullOrEmpty(imgDb.FK_MapData))
							{
								...
								imgDb.EleID = String.valueOf(wk.getOID());
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	private void CC(Node node)
	{
		...
		if (DataType.IsNullOrEmpty(ccMsg) == false)
		{
			...
			this.AddToTrack(ActionType.CC, WebUser.No, WebUser.Name, node.getNodeID(), node.getName(), ccMsg1 + ccMsg2, node);
		}
	}
	/** 
	 处理OrderTeamup退回模式
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		{
			String str = BP.WF.Glo.multilingual("@判断流程({0})完成条件出现错误:{1}.", "WorkNode", "error_workflow_complete_condition", ex.StackTrace, this.getHisNode().getName());
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	*/
	private String Func_DoSetThisWorkOver()
	{
		...
		this.rptGe.SetValByKey(GERptAttr.FlowDaySpan, DataType.GeTimeLimits(rptGe.getFlowStartRDT(), DataType.CurrentDataTime));

		//如果两个物理表不想等.
		if (this.getHisWork().EnMap.PhysicsTable != this.rptGe.getEnMap().PhysicsTable)
		{
			// 更新状态。
			this.getHisWork().SetValByKey("CDT", DataType.CurrentDataTime);
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		if (town.getHisNode().getTodolistModel() == TodolistModel.Order && current_gwls.size() > 1)
		{
			...
			int idx = 0;
			for (GenerWorkerList gwl : current_gwls)
			{
				idx++;
				if (idx == 1)
				{
		...
		if ((town.getHisNode().getTodolistModel() == TodolistModel.Teamup || town.getHisNode().getTodolistModel() == TodolistModel.TeamupGroupLeader) && current_gwls.size() > 1)
		{
			...
			if (town.getHisNode().getFWCOrderModel() == 1)
			{
				...
				for (GenerWorkerList gwl : current_gwls)
				{
					...
					BP.WF.Dev2Interface.WriteTrack(this.getHisFlow().No, town.getHisNode().getNodeID(), town.getHisNode().getName(), this.getWorkID(), town.getHisWork().getFID(), "", ActionType.WorkCheck, null, null, null, gwl.getFK_Emp(), gwl.getFK_EmpText(), gwl.getFK_Emp(), gwl.getFK_EmpText(), rdt);
				}
			}
		}
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		if (this.getHisWork().EnMap.PhysicsTable != toWK.EnMap.PhysicsTable)
		{
			...
			{
				Log.DefaultLogWriteLineInfo(BP.WF.Glo.multilingual("@出现SQL异常! 可能是没有修复表或者重复发送.详情:{0}.", "WorkNode", "sql_exception_1", ex.getMessage()));
				try
				{
				...
				{
					Log.DefaultLogWriteLineInfo(BP.WF.Glo.multilingual("@保存工作出错! 详情:{0}.", "WorkNode", "error_saving_data", ex1.getMessage()));
					throw new RuntimeException(BP.WF.Glo.multilingual("@保存工作出错! 详情:{0}.", "WorkNode", "error_saving_data", toWK.EnDesc + ex1.getMessage()));
				}
			}
		}
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		if (this.getHisNode().getHisFormType() != NodeFormType.SDKForm || 1 == 1)
		{
			if (this.getHisNode().getIsStartNode())
			{
				...
				this.addMsg(SendReturnMsgFlag.ToEmpExt, null, "@<a href='./WorkOpt/UnSend.htm?DoType=UnSend&UserNo=" + WebUser.No + "&SID=" + WebUser.SID + "&WorkID=" + this.getHisWork().getOID() + "&FK_Flow=" + toND.getFK_Flow() + "' ><img src='./Img/Action/UnSend.png' border=0/>撤销本次发送</a>", SendReturnMsgType.Info);
			}
			else
			{
				this.addMsg(SendReturnMsgFlag.ToEmpExt, null, "@<a href='./WorkOpt/UnSend.htm?DoType=UnSend&UserNo=" + WebUser.No + "&SID=" + WebUser.SID + "&WorkID=" + this.getHisWork().getOID() + "&FK_Flow=" + toND.getFK_Flow() + "' ><img src='./Img/Action/UnSend.png' border=0 />撤销本次发送</a> ", SendReturnMsgType.Info);
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		*/
		...
		if (toNode.getHisDeliveryWay() == DeliveryWay.ByDtlAsSubThreadEmps)
		{
			...
			for (MapDtl dtl : dtlsFrom)
			{
				//加上顺序，防止变化，人员编号变化，处理明细表中接收人重复的问题。
				String sql = "SELECT * FROM " + dtl.PTable + " WHERE RefPK=" + this.getWorkID() + " ORDER BY OID";
				dtWork = BP.DA.DBAccess.RunSQLReturnTable(sql);
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
			...
			else
			{
				...
				if (this.IsHaveSubThreadGroupMark == true)
				{
					...
					{
						mywk.setOID(Long.parseLong(dt.Rows[0][0].toString())); //使用该分组下的，已经注册过的分组的WorkID，而非产生一个新的WorkID。
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
			...
			if (isHaveFLDtl == true)
			{
				...
				for (MapDtl dtl : this.getHisWork().getHisMapDtls())
				{
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
				...
				if (athDBs.size() > 0)
				{
					...
					for (FrmAttachmentDB athDB : athDBs)
					{
						FrmAttachmentDB athDB_N = new FrmAttachmentDB();
						athDB_N.Copy(athDB);
						athDB_N.FK_MapData = "ND" + toNode.getNodeID();
						athDB_N.RefPKVal = String.valueOf(mywk.getOID());
						athDB_N.FK_FrmAttachment = athDB_N.FK_FrmAttachment.Replace("ND" + this.getHisNode().getNodeID(), "ND" + toNode.getNodeID());

						if (athDB_N.HisAttachmentUploadType == AttachmentUploadType.Single)
						{
							//注意如果是单附件主键的命名规则不能变化，否则会导致与前台约定获取数据错误。
							athDB_N.MyPK = athDB_N.FK_FrmAttachment + "_" + mywk.getOID();
							try
							{
								athDB_N.DirectInsert();
							}
					...
					for (FrmImg img : this.getHisNode().getMapData().FrmImgs)
					{
						...
						if (eleDBs.size() > 0)
						{
							...
							for (FrmEleDB eleDB : eleDBs)
							{
								FrmEleDB eleDB_N = new FrmEleDB();
								eleDB_N.Copy(eleDB);
								eleDB_N.FK_MapData = img.EnPK.Replace("ND" + this.getHisNode().getNodeID(), "ND" + toNode.getNodeID());
								eleDB_N.RefPKVal = img.EnPK.Replace("ND" + this.getHisNode().getNodeID(), "ND" + toNode.getNodeID());
								eleDB_N.EleID = String.valueOf(mywk.getOID());
								eleDB_N.GenerPKVal();
								eleDB_N.Save();
							}
						}
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
				...
				if (this.getHisNode().getMapData().FrmImgAths.size() > 0)
				{
					...
					if (frmImgAthDBs.size() > 0)
					{
						...
						/*说明当前节点有附件数据*/
						for (FrmImgAthDB imgAthDB : frmImgAthDBs)
						{
							FrmImgAthDB imgAthDB_N = new FrmImgAthDB();
							imgAthDB_N.Copy(imgAthDB);
							imgAthDB_N.FK_MapData = "ND" + toNode.getNodeID();
							imgAthDB_N.RefPKVal = String.valueOf(mywk.getOID());
							imgAthDB_N.FK_FrmImgAth = imgAthDB_N.FK_FrmImgAth.Replace("ND" + this.getHisNode().getNodeID(), "ND" + toNode.getNodeID());
							imgAthDB_N.Save();
						}
					}
				}
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
				...
				if (dtlsFrom.size() > 0 && dtlsTo.size() > 0)
				{
					...
					for (Sys.MapDtl dtl : dtlsFrom)
					{
						...
						int unPass = 0;
						DBAccess.RunSQL("DELETE FROM " + toDtl.PTable + " WHERE RefPK=" + dbStr + "RefPK", "RefPK", mywk.getOID());
						for (GEDtl gedtl : gedtls)
						{
							BP.Sys.GEDtl dtCopy = new GEDtl(toDtl.No);
							dtCopy.Copy(gedtl);
							dtCopy.FK_MapDtl = toDtl.No;
							dtCopy.RefPK = String.valueOf(mywk.getOID());
							dtCopy.OID = 0;
							dtCopy.Insert();
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
			...
			if (gwf.RetrieveFromDBSources() == 0)
			{
				...
				gwf.setSender(BP.WF.Glo.DealUserInfoShowModel(WebUser.No, WebUser.Name));
			...
			if (isGroupMarkWorklist == false)
			{
				...
				flGwl.setFK_Emp(WebUser.No);
				flGwl.setFK_EmpText(WebUser.Name);
				flGwl.setFK_Node(this.getHisNode().getNodeID());
				flGwl.setSender(WebUser.No + "," + WebUser.Name);
				// flGwl.GroupMark = "";
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		if (this.getHisNode().getIsStartNode())
		{
			...
			this.addMsg("UnDo", "@<a href='./WorkOpt/UnSend.htm?DoType=UnSend&UserNo=" + WebUser.No + "&SID=" + WebUser.SID + "&WorkID=" + this.getWorkID() + "&FK_Flow=" + toNode.getFK_Flow() + "' ><img src='./Img/Action/UnSend.png' border=0/>撤销本次发送</a>.");
		}
		else
		{
			this.addMsg("UnDo", "@<a href='./WorkOpt/UnSend.htm?DoType=UnSend&UserNo=" + WebUser.No + "&SID=" + WebUser.SID + "&WorkID=" + this.getWorkID() + "&FK_Flow=" + toNode.getFK_Flow() + "' ><img src='./Img/Action/UnSend.png' border=0/>撤销本次发送</a>.");
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		{
			//	新增加轨迹
			GenerWorkerList gwl = new GenerWorkerList(this.getHisWork().getFID(), toNode.getNodeID(), WebUser.No);
			ActionType at = ActionType.SubThreadForward;
			this.AddToTrack(at, gwl, BP.WF.Glo.multilingual("子线程", "WorkNode", "sub_thread"), this.town.getHisWork().getOID());
		}
...
///#warning 对于多个分合流点可能会有问题。
		...
		BigDecimal numPassed = gwf.GetParaInt("ThreadCount");

		BigDecimal passRate = numPassed.divide(numAll.multiply(100));
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		if (this.SendNodeWFState == WFState.ReturnSta)
		{
			...
			if (dt.Rows.size() != 0)
			{
				...
				this.JumpToNode = new Node(Integer.parseInt(dt.Rows[0]["ReturnNode"].toString()));
			...
			case Ordinary: // 1： 普通节点向下发送的
				...
				switch (toND.getHisRunModel())
				{
					...
					case SubThread: //1-5 普通节to子线程点
						throw new RuntimeException(BP.WF.Glo.multilingual("@流程设计错误: 普通节点下面不能连接子线程节点{0}", "WorkNode", "workflow_error_3", toND.getName()));
					default:
		...
		if (this.getHisNode().getMapData().FrmAttachments.size() > 0)
		{
			...
			if (athDBs.size() > 0)
			{
				...
				for (FrmAttachmentDB athDB : athDBs)
				{
					FrmAttachmentDB athDB_N = new FrmAttachmentDB();
					athDB_N.Copy(athDB);
					athDB_N.FK_MapData = "ND" + toND.getNodeID();
					athDB_N.RefPKVal = String.valueOf(this.getHisWork().getOID());
					athDB_N.FK_FrmAttachment = athDB_N.FK_FrmAttachment.Replace("ND" + this.getHisNode().getNodeID(), "ND" + toND.getNodeID());

					if (athDB_N.HisAttachmentUploadType == AttachmentUploadType.Single)
					{
						/*如果是单附件.*/
						athDB_N.MyPK = athDB_N.FK_FrmAttachment + "_" + this.getHisWork().getOID();
					...
					else
					{
						...
						athDB_N.MyPK = athDB_N.UploadGUID + "_" + athDB_N.FK_MapData + "_" + toWK.getOID();
						try
						{
							athDB_N.Insert();
						}
		...
		if (this.getHisNode().getMapData().FrmImgAths.size() > 0)
		{
			...
			if (athDBs.size() > 0)
			{
				...
				for (FrmImgAthDB athDB : athDBs)
				{
					...
					athDB_N.RefPKVal = String.valueOf(this.getWorkID());
					athDB_N.MyPK = this.getWorkID() + "_" + idx + "_" + athDB_N.FK_MapData;
					athDB_N.FK_FrmImgAth = athDB_N.FK_FrmImgAth.Replace("ND" + this.getHisNode().getNodeID(), "ND" + toND.getNodeID());
					athDB_N.Save();
				}
			}
		}
			...
			for (FrmImg img : this.getHisNode().getMapData().FrmImgs)
			{
				...
				if (eleDBs.size() > 0)
				{
					...
					for (FrmEleDB eleDB : eleDBs)
					{
						FrmEleDB eleDB_N = new FrmEleDB();
						eleDB_N.Copy(eleDB);
						eleDB_N.FK_MapData = img.EnPK.Replace("ND" + this.getHisNode().getNodeID(), "ND" + toND.getNodeID());
						eleDB_N.RefPKVal = img.EnPK.Replace("ND" + this.getHisNode().getNodeID(), "ND" + toND.getNodeID());
						eleDB_N.GenerPKVal();
						eleDB_N.Save();
					}
				}
		...
		if (dtls.size() > 0)
		{
			...
			for (Sys.MapDtl dtl : dtls)
			{
				...
				for (MapDtl todtl : toDtls)
				{
					...
					try
					{
						toDtlName = todtl.HisGEDtl.FK_MapDtl.substring(todtl.HisGEDtl.FK_MapDtl.indexOf("Dtl"), todtl.HisGEDtl.FK_MapDtl.Length);
						dtlName = dtl.HisGEDtl.FK_MapDtl.substring(dtl.HisGEDtl.FK_MapDtl.indexOf("Dtl"), dtl.HisGEDtl.FK_MapDtl.Length);
					}
					...
					if (toDtl.IsEnableAthM)
					{
						...
						if (athDBs.size() > 0)
						{
							...
							for (FrmAttachmentDB athDB : athDBs)
							{
								...
								athDB_N.Copy(athDB);
								athDB_N.FK_MapData = toDtl.No;
								athDB_N.MyPK = athDB.MyPK + "_" + dtCopy.OID + "_" + String.valueOf(i);
								athDB_N.FK_FrmAttachment = athDB_N.FK_FrmAttachment.Replace("ND" + this.getHisNode().getNodeID(), "ND" + toND.getNodeID());
								athDB_N.RefPKVal = dtCopy.OID.toString();
								try
								{
...
///#warning 记录日志.
				if (gedtls.size() != deBugNumCopy)
				{
					...
					//记录日志.
					Log.DefaultLogWriteLineInfo(recDtlLog);
					throw new RuntimeException(BP.WF.Glo.multilingual("@系统出现错误,请将如下信息反馈给管理员,谢谢。技术信息:{0}.", "WorkNode", "system_error", recDtlLog));
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	*/
	public final void DealEvalUn()
	{
		...
		{
			eval.MyPK = this.getWorkID() + "_" + this.getHisNode().getNodeID();
			eval.Delete();
		}

		// 分合流的情况，它是明细表产生的质量评价。
		MapDtls dtls = this.getHisNode().getMapData().MapDtls;
		for (MapDtl dtl : dtls)
		{
			if (dtl.IsHLDtl == false)
			{
	...
	*/
	public final void DealEval()
	{
		...
		if (this.getHisNode().getIsFLHL() == false)
		{
			...
			eval.setRDT(DataType.CurrentDataTime);
			eval.setFK_NY(DataType.CurrentYearMonth);
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
			...
			dtlEn.RefPK = String.valueOf(this.getHisWork().getFID());
			dtlEn.FID = 0;
	...
	/** 
	 ...
	 @return 
	*/
	private boolean CheckFrmIsFullCheckNote()
	{
		//检查是否写入了审核意见.
	...
	*/
	public final boolean CheckFrmIsNotNull()
	{
		...
		if (this.getHisNode().getHisFormType() == NodeFormType.SheetTree)
		{
			...
			for (FrmNode item : nds)
			{
				...
				if (item.getFrmSln() == FrmSln.Self)
				{
					...
					ffs1.Retrieve(FrmFieldAttr.FK_Node, this.getHisNode().getNodeID(), FrmFieldAttr.FK_MapData, md.No);
					//获取整合后的mapAttrs
					for (FrmField frmField : ffs1)
					{
						for (MapAttr mapAttr : mapAttrs)
						{
							if (frmField.getKeyOfEn().equals(mapAttr.KeyOfEn))
							{
								mapAttr.UIIsInput = frmField.getIsNotNull();
								break;
							}
						}
					}
				...
				for (MapAttr mapAttr : mapAttrs)
				{
					...
					String str = row[mapAttr.KeyOfEn] == null ? "" : row[mapAttr.KeyOfEn].toString();
					/*如果是检查不能为空 */
					if (str == null || DataType.IsNullOrEmpty(str) == true || str.trim().equals(""))
					{
						err += BP.WF.Glo.multilingual("@表单{0}字段{1},{2}不能为空.", "WorkNode", "form_field_must_not_be_null_1", item.getFK_Frm(), mapAttr.KeyOfEn, mapAttr.Name);
					}
				}
			}
		...
		{
			MapAttrs attrs = this.getHisNode().getMapData().MapAttrs;
			Row row = this.getHisWork().Row;
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
				...
				if (ath.ReadRole != 0)
				{
					//查询出来当前的数据.
					GenerWorkerList gwl = new GenerWorkerList();
					gwl.Retrieve(GenerWorkerListAttr.WorkID, this.getWorkID(), GenerWorkerListAttr.FK_Emp, WebUser.No, GenerWorkerListAttr.FK_Node, this.getHisNode().getNodeID());
					...
					{
						//获得当前节点的上传附件.
						FrmAttachmentDBs dbs = BP.WF.Glo.GenerFrmAttachmentDBs(ath, String.valueOf(this.getWorkID()), ath.MyPK, this.getWorkID(), 0, 0, false);
						...
						if (DataType.IsNullOrEmpty(errFileUnRead) == false)
						{
							...
							if (ath.ReadRole == 2)
							{
								...
								log.MyPK = this.getWorkID() + "_" + this.getHisNode().getNodeID() + "_" + WebUser.No;
								log.Delete();

								log.setFK_Emp(WebUser.No);
								log.setFK_EmpDept(WebUser.FK_Dept);
								log.setFK_EmpDeptName(WebUser.FK_DeptName);
								log.setFK_Flow(this.getHisNode().getFK_Flow());
								log.setFlowName(this.getHisFlow().Name);

								log.setFK_Node(this.getHisNode().getNodeID());
								log.setFlowName(this.getHisFlow().Name);
								log.setSendDT(DataType.CurrentDataTime);
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		for (FrmNode item : frmNDs)
		{
			...
			for (FrmField ff : ffs)
			{
				...
				//获得数据.
				String val = "";
				val = dt.Rows[0][ff.getKeyOfEn()].toString();
				...
				if (ff.getIsNotNull() == true)
				{
					...
					{
						err += BP.WF.Glo.multilingual("@表单{0}字段{1},{2}不能为空.", "WorkNode", "form_field_must_not_be_null_1", md.Name, ff.getKeyOfEn(), ff.getName());
					}
	...
	*/
	private String DealAlertZhuChiRen()
	{
		...
		BP.WF.Dev2Interface.Port_SendMsg(gwl.getFK_Emp(), str1, str2, "HuiQian" + this.getWorkID() + "_" + WebUser.No, "HuiQian", getHisGenerWorkFlow().getFK_Flow(), this.getHisGenerWorkFlow().getFK_Node(), this.getWorkID(), 0);
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		if (this.getHisNode().getHisCHWay() == CHWay.ByTime)
		{
			...
			//获得两个时间间隔.
			LocalDateTime t_lastDTOfHuiQian = DataType.ParseSysDate2DateTime(lastDTOfHuiQian);
			LocalDateTime t_dtOfToZhuChiRen = DataType.ParseSysDate2DateTime(dtOfToZhuChiRen);
			...
			TimeSpan tsLimt = new TimeSpan(hour, this.getHisNode().getTimeLimitMM(), 0);
			...
			dtNow = dtNow.plusHours(myLeftTS.TotalHours);
	...
	*/
	public final boolean DealTeamUpNode()
	{
		...
		for (GenerWorkerList item : gwls)
		{
			...
			{
				if (item.getFK_Emp().equals(WebUser.No) == false)
				{
		...
		if (num == 1)
		{
			...
			{
				this.getHisGenerWorkFlow().setSender(BP.WF.Glo.DealUserInfoShowModel(BP.Web.WebUser.No, BP.Web.WebUser.Name));
				this.getHisGenerWorkFlow().setTodoEmpsNum(1);
				this.getHisGenerWorkFlow().setTodoEmps(WebUser.Name + ";");
			}
			else
			{
		...
		{
			if (gwl.getFK_Emp().equals(WebUser.No) == false)
			{
				continue;
			}
			...
			if (this.getHisGenerWorkFlow().getHuiQianTaskSta() != HuiQianTaskSta.None)
			{
				this.AddToTrack(ActionType.TeampUp, gwl.getFK_Emp(), todoEmps, this.getHisNode().getNodeID(), this.getHisNode().getName(), BP.WF.Glo.multilingual("会签", "WorkNode", "cross_signing"));
			}
			else
			{
				this.AddToTrack(ActionType.TeampUp, gwl.getFK_Emp(), todoEmps, this.getHisNode().getNodeID(), this.getHisNode().getName(), BP.WF.Glo.multilingual("协作发送", "WorkNode", "cross_signing"));
			}
	...
	/** 
	 如果是协作
	*/
	public final boolean DealTeamupGroupLeader()
	{
		GenerWorkerLists gwls = new GenerWorkerLists();
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		{
			if (this.getHisGenerWorkFlow().getTodoEmps().contains(BP.Web.WebUser.No + "," + BP.Web.WebUser.Name + ";") == true)
			{
				...
				for (GenerWorkerList item : gwls)
				{
						if (item.getIsPassInt() == 0 || item.getIsPassInt() == 90)
						{
							...
							//任意组长
							if (this.getHisNode().getHuiQianLeaderRole() == HuiQianLeaderRole.EveryOneMain && (huiQianZhuChiRen.equals(WebUser.No) || item.getFK_Emp().equals(WebUser.No)))
							{
								num++;
							}

							//最后一个组长
							if (this.getHisNode().getHuiQianLeaderRole() == HuiQianLeaderRole.LastOneMain && (huiQianZhuChiRen.equals(WebUser.No) || DataType.IsNullOrEmpty(huiQianZhuChiRen) == true))
							{
								num++;
							}
						}
				...
				if (num == 1)
				{
					...
					{
						String sql = "Delete FROM WF_GenerWorkerList WHERE WorkID=" + this.getWorkID() + " AND FK_Node=" + this.getHisNode().getNodeID() + " AND IsPass=0";
						DBAccess.RunSQL(sql);
				...
				for (GenerWorkerList gwl : gwls)
				{
					if (!gwl.getFK_Emp().equals(WebUser.No))
					{
						continue;
					}
				...
				if (SystemConfig.CustomerNo.equals("LIMS"))
				{
					this.getHisGenerWorkFlow().setSender(BP.WF.Glo.DealUserInfoShowModel(BP.Web.WebUser.No, BP.Web.WebUser.Name));
					this.getHisGenerWorkFlow().setHuiQianTaskSta(HuiQianTaskSta.None);
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		for (GenerWorkerList gwl : gwls)
		{
			...
			this.AddToTrack(ActionType.TeampUp, gwl.getFK_Emp(), todoEmps1, this.getHisNode().getNodeID(), this.getHisNode().getName(), "协作发送");
	...
	public final boolean DealOradeNode()
	{
		...
		for (GenerWorkerList gwl : gwls)
		{
			if (gwl.getIsPassInt() > 10)
			{
				...
				//执行更新.
				if (this.getHisGenerWorkFlow().getEmps().contains("@" + WebUser.No + "@") == false)
				{
					this.getHisGenerWorkFlow().setEmps(this.getHisGenerWorkFlow().getEmps() + WebUser.No + "@");
				}
	...
	*/
	private void CheckBlockModel()
	{
		...
		if (tangible.StringHelper.isNullOrWhiteSpace(this.getHisNode().getBlockAlert()))
		{
		...
		{
			/*按 sql 判断阻塞*/
			BigDecimal d = DBAccess.RunSQLReturnValDecimal(Glo.DealExp(this.getHisNode().getBlockExp(), this.rptGe, null), 0, 1);
			//如果值大于0进行阻塞
			if (d.compareTo(0) > 0)
			{
		...
		if (this.getHisNode().getBlockModel() == BlockModel.ByExp)
		{
			...
			String key = tangible.StringHelper.trimStart(strs[0].trim(), '@');
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
			...
			if (node.getHisDeliveryWay() == DeliveryWay.ByDeptAndStation)
			{
				...
				sql = "SELECT pdes.FK_Emp AS No"
					  ...
					  + " AND wnd.FK_Node = " + node.getNodeID() + " INNER JOIN WF_NodeStation wns ON  wns.FK_Station = pdes.FK_Station"
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		subFlow.MyPK = this.getHisNode().getNodeID() + "_" + node.getFK_Flow() + "_" + 2;
		...
		{
			workid = BP.WF.Dev2Interface.Node_CreateBlankWork(node.getFK_Flow(), null, null, toEmpIDs, null, this.getWorkID(), 0, this.getHisNode().getFK_Flow(), this.getHisNode().getNodeID(), BP.Web.WebUser.No, 0, null);
		}
		else if (subFlow.getHisSubFlowModel() == SubFlowModel.SameLevel) //平级子流程
		{
			workid = BP.WF.Dev2Interface.Node_CreateBlankWork(node.getFK_Flow(), null, null, toEmpIDs, null, this.getHisGenerWorkFlow().getPWorkID(), 0, this.getHisGenerWorkFlow().getPFlowNo(), this.getHisGenerWorkFlow().getPNodeID(), this.getHisGenerWorkFlow().getPEmp(), 0, null);
			//存储同级子流程的信息
		...
		int count = gwl.Retrieve(GenerWorkerListAttr.WorkID, workid, GenerWorkerListAttr.FK_Node, node.getNodeID(), GenerWorkerListAttr.FK_Emp, toEmpIDs);
		if (count == 0)
		{
		...
		//设置消息.
		this.addMsg("Msg1", BP.WF.Glo.multilingual("子流程({0})已经启动,发送给({1})处理人.", "sub_wf_started", node.getFlowName(), toEmpIDs));
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		{
			throw new RuntimeException(BP.WF.Glo.multilingual("@当前工作您已经处理完成，或者您({0} {1})没有处理当前工作的权限.", "WorkNode", "current_work_completed", this.getExecer(), this.getExecerName()));
		}
		...
		String sendWhen = this.getHisFlow().DoFlowEventEntity(EventListOfNode.SendWhen, this.getHisNode(), this.getHisWork(), null, null, toNodeID, jumpToEmp);
		...
		if (this.getHisNode().getIsStartNode() == false && this.getHisGenerWorkFlow().getWFState() == WFState.Askfor)
		{
			...
			for (GenerWorkerList item : gwls)
			{
				if (item.getIsPassInt() == AskforHelpSta.AfterDealSend.getValue())
				{
					...
					//写入日志.
					this.AddToTrack(ActionType.ForwardAskfor, item.getFK_Emp(), item.getFK_EmpText(), this.getHisNode().getNodeID(), this.getHisNode().getName(), BP.WF.Glo.multilingual("加签后向下发送，直接发送给下一步处理人。", "WorkNode", "send_to_next"));
				...
				if (item.getIsPassInt() == AskforHelpSta.AfterDealSendByWorker.getValue())
				{
					...
					//写入日志.
					this.AddToTrack(ActionType.ForwardAskfor, item.getFK_Emp(), item.getFK_EmpText(), this.getHisNode().getNodeID(), this.getHisNode().getName(), BP.WF.Glo.multilingual("回复意见:{0}.", "WorkNode", "reply_comments", replyInfo));
					...
					//加入提示信息.
					this.addMsg(SendReturnMsgFlag.SendSuccessMsg, BP.WF.Glo.multilingual("已经转给加签的发起人({0},{1})", "WorkNode", "send_to_the_operator", item.getFK_Emp().toString(), item.getFK_EmpText()), SendReturnMsgType.Info);
		...
		if (this.getHisNode().getIsHL() || this.getHisNode().getHisRunModel() == RunModel.FHL)
		{
			...
			if (dtWL.Rows.size() != 0)
			{
				if (this.getHisNode().getThreadKillRole() == ThreadKillRole.None || this.getHisNode().getThreadKillRole() == ThreadKillRole.ByHand)
				{
					...
					{
						String op = BP.WF.Glo.multilingual("@操作员编号:{0},{1}", "WorkNode", "current_operator", dr.get("FK_Emp").toString(), dr.get("FK_EmpText").toString());
						String nd = BP.WF.Glo.multilingual("停留节点:{0}.", "WorkNode", "current_node", dr.get("FK_NodeText").toString());
				...
				if (this.getHisNode().getThreadKillRole() == ThreadKillRole.ByAuto)
				{
					...
					for (DataRow dr : dtWL.Rows)
					{
						BP.WF.Dev2Interface.Flow_DeleteSubThread(this.getHisFlow().No, Long.parseLong(dr.get(0).toString()), BP.WF.Glo.multilingual("合流点发送时自动删除", "WorkNode", "auto_delete"));
					}
				}
			}
		}
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		try
		{
			...
			this.getHisGenerWorkFlow().setSender(BP.WF.Glo.DealUserInfoShowModel(WebUser.No, WebUser.Name));
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
			...
			if (this.getHisGenerWorkFlow().getWFState() == WFState.ReturnSta)
			{
				...
				if (mydt.Rows.size() != 0)
				{
					...
					/*确认这次退回，是退回并原路返回 ,  在这里初始化它的工作人员, 与将要发送的节点. */
					this.JumpToNode = new Node(Integer.parseInt(mydt.Rows[0]["ReturnNode"].toString()));
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
				...
				{
					//执行自动抄送
					String ccMsg1 = WorkFlowBuessRole.DoCCAuto(this.getHisNode(), this.rptGe, this.getWorkID(), this.getHisWork().getFID());

					//按照指定的字段抄送.
					String ccMsg2 = WorkFlowBuessRole.DoCCByEmps(this.getHisNode(), this.rptGe, this.getWorkID(), this.getHisWork().getFID());
					//手工抄送
					if (this.getHisNode().getHisCCRole() == CCRole.HandCC)
					{
						//获取抄送人员列表
					...
					if (DataType.IsNullOrEmpty(ccMsg) == false)
					{
						...
						this.AddToTrack(ActionType.CC, WebUser.No, WebUser.Name, this.getHisNode().getNodeID(), this.getHisNode().getName(), ccMsg1 + ccMsg2, this.getHisNode());
					}
				}

				//判断当前流程是否子流程，是否启用该流程结束后，主流程自动运行到下一节点@yuan
				String msg = BP.WF.Dev2Interface.FlowOverAutoSendParentOrSameLevelFlow(this.getHisGenerWorkFlow(), this.getHisFlow());
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
						...
						if (func.getHisBillFileType() == BillFileType.PDF)
						{
							...
							{
								this.addMsg("RptError", BP.WF.Glo.multilingual("生成报表数据错误:{0}.", "WorkNode", "rpt_error", ex.getMessage()));
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
							///#region 保存单据
						Bill bill = new Bill();
						bill.MyPK = this.getHisWork().getFID() + "_" + this.getHisWork().getOID() + "_" + this.getHisNode().getNodeID() + "_" + func.getNo();
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
				...
				if (this.town != null && this.HisRememberMe != null && this.HisRememberMe.getEmps().contains("@" + WebUser.No + "@") == true)
				{
					String url = "MyFlow.htm?FK_Flow=" + this.getHisFlow().No + "&WorkID=" + this.getWorkID() + "&FK_Node=" + town.getHisNode().getNodeID() + "&FID=" + this.rptGe.getFID();
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
				...
				{
					/*说明有一些变量在系统运行里面.*/
					Object tempVar = msgOfSend.Clone();
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
			...
			if (this.getHisNode().getIsStartNode())
			{
				...
				{
					BP.WF.Dev2Interface.SetParentInfo(this.getHisFlow().No, this.getWorkID(), this.rptGe.getPWorkID());
					...
					String paras = "@CFlowNo=" + this.getHisFlow().No + "@CWorkID=" + this.getWorkID();

					Glo.AddToTrack(ActionType.StartChildenFlow, rptGe.getPFlowNo(), rptGe.getPWorkID(), fid, pND.getNodeID(), pND.getName(), WebUser.No, WebUser.Name, pND.getNodeID(), pND.getName(), WebUser.No, WebUser.Name, "<a href='" + SystemConfig.HostURLOfBS + "/WF/WFRpt.htm?FK_Flow=" + this.getHisFlow().No + "&WorkID=" + this.getWorkID() + "' target=_blank >打开子流程</a>", paras);
				}
				else if (SystemConfig.IsBSsystem == true)
				{
					...
					if (DataType.IsNullOrEmpty(pflowNo) == false)
					{
						...
						// 设置成父流程关系.
						BP.WF.Dev2Interface.SetParentInfo(this.getHisFlow().No, this.getWorkID(), Long.parseLong(pWorkID));
						...
						String paras = "@CFlowNo=" + this.getHisFlow().No + "@CWorkID=" + this.getWorkID();
						Glo.AddToTrack(ActionType.StartChildenFlow, pflowNo, Long.parseLong(pWorkID), Long.parseLong(String.valueOf(fid)), pND.getNodeID(), pND.getName(), WebUser.No, WebUser.Name, pND.getNodeID(), pND.getName(), WebUser.No, WebUser.Name, "<a href='" + SystemConfig.HostURLOfBS + "/WF/WFRpt.htm?FK_Flow=" + this.getHisFlow().No + "&WorkID=" + this.getWorkID() + "' target=_blank >" + BP.WF.Glo.multilingual("打开子流程", "WorkNode", "open_sub_wf") + "</a>", paras);
					}
				}
			}
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	/** 
	 自动启动子流程
	*/
	public final void CallAutoSubFlow(Node nd, int invokeTime)
	{
		//自动发起流程的数量.
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
				...
				{
					//创建workid.
					long subWorkID = BP.WF.Dev2Interface.Node_CreateBlankWork(sub.getSubFlowNo(), WebUser.No);


					//执行保存.
					BP.WF.Dev2Interface.Node_SaveWork(sub.getSubFlowNo(), Integer.parseInt(sub.getSubFlowNo() + "01"), subWorkID, this.rptGe.Row);
					...
					//设置父子关系.
					BP.WF.Dev2Interface.SetParentInfo(sub.getSubFlowNo(), subWorkID, this.getHisGenerWorkFlow().getWorkID(), WebUser.No, nd.getNodeID());
				...
				if (sub.getSendModel() == 1)
				{
					...
					//设置父子关系.
					BP.WF.Dev2Interface.SetParentInfo(sub.getSubFlowNo(), subWorkID, this.getHisGenerWorkFlow().getWorkID());

					//执行发送到下一个环节..
					SendReturnObjs sendObjs = BP.WF.Dev2Interface.Node_SendWork(sub.getSubFlowNo(), subWorkID, this.rptGe.Row, null);
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
				...
				if (sub.getIsEnableSameLevelNode() == true)
				{
					...
					for (String val : nodes)
					{
						...
						{
							String sql = "SELECT COUNT(*) as Num FROM WF_GenerWorkerList WHERE WorkID=" + gwfSub.getWorkID() + " AND FK_Flow='" + flowNode[0] + "' AND FK_Node=" + Integer.parseInt(flowNode[1]) + " AND IsEnable=1 AND IsPass=1";
							if (DBAccess.RunSQLReturnValInt(sql) == 0)
							{
								isHave = true;
								break; //不能启动.
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
				...
				if (sub.getSendModel() == 0)
				{
					...
					//设置父子关系.
					BP.WF.Dev2Interface.SetParentInfo(sub.getSubFlowNo(), subWorkID, this.getHisGenerWorkFlow().getPWorkID(), WebUser.No, nd.getNodeID());
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		try
		{
			...
			{
				for (String key : this.SendHTOfTemp.keySet())
				{
					if (rptGe.Row.ContainsKey(key) == true)
					{
						this.rptGe.Row[key] = this.SendHTOfTemp.get(key).toString();
					}
					else
					{
						this.rptGe.Row.Add(key, this.SendHTOfTemp.get(key).toString());
					}
				}
			}
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		if (this.town != null && this.town.getHisNode().getHisNodeWorkType() == NodeWorkType.SubThreadWork && this.town.getHisNode().getHisSubThreadType() == SubThreadType.SameSheet)
		{
			...
			//删除子线程数据.
			if (BP.DA.DBAccess.IsExitsObject(this.town.getHisWork().EnMap.PhysicsTable) == true)
			{
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		{
			//删除发生的日志.
			DBAccess.RunSQL("DELETE FROM ND" + Integer.parseInt(this.getHisFlow().No) + "Track WHERE WorkID=" + this.getWorkID() + " AND NDFrom=" + this.getHisNode().getNodeID() + " AND ActionType=" + ActionType.Forward.getValue());
			...
			{
				String ndTrack = "ND" + Integer.parseInt(this.getHisFlow().No) + "Track";
				...
				for (DataRow dr : dt.Rows)
				{
					...
					GenerWorkerList gwl = new GenerWorkerList();
					gwl.setWorkID(this.getWorkID());
					gwl.setFK_Flow(this.getHisFlow().No);
		   ...
		   if (gwf.getWFState() != 0 || gwf.getFK_Node() != this.getHisNode().getNodeID())
		   {
				/* 如果这两项其中有一项有变化。*/
			...
			for (Node nd : nds)
			{
				...
				if (this.getHisFlow().getPTable().equals(mwk.EnMap.PhysicsTable) || mwk.EnMap.PhysicsTable == this.getHisWork().EnMap.PhysicsTable)
				{
					continue;
				}
	...
	private void InitStartWorkDataV2()
	{
		...
		if (SystemConfig.IsBSsystem)
		{
			...
			if (DataType.IsNullOrEmpty(fk_nodeFrom) == false)
			{
				...
				String st2 = BP.WF.Glo.multilingual("发起子流程:{0}", "WorkNode", "start_sub_wf", this.getHisFlow().Name);
				BP.WF.Dev2Interface.WriteTrack(this.getHisFlow().No, this.getHisNode().getNodeID(), this.getHisNode().getName(), this.getWorkID(), 0, st1, ActionType.CallChildenFlow, "@PWorkID=" + PWorkID + "@PFlowNo=" + ndFrom.getHisFlow().No, st2, null);
			}
		}
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		if (this.title == null)
		{
			if (this.getHisFlow().getTitleRole().equals("@OutPara") || DataType.IsNullOrEmpty(this.getHisFlow().getTitleRole()) == true)
			{
				...
				{
					gwf.setTitle(BP.WF.Glo.multilingual("{0},{1}在{2}发起", "WorkNode", "start_wf_title", this.getExecer(), this.getExecerName(), DataType.CurrentDataTimeCN));
				}
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		gwf.setRDT(DataType.CurrentDataTimess); // this.HisWork.RDT;
		gwf.setStarter(this.getExecer());
		gwf.setStarterName(this.getExecerName());
		...
		gwf.setFK_Node(this.getHisNode().getNodeID());
		gwf.setNodeName(this.getHisNode().getName());
		gwf.setFK_Dept(this.getHisWork().getRecOfEmp().FK_Dept);
		gwf.setDeptName(this.getHisWork().getRecOfEmp().FK_DeptText);
		...
		if (this.getHisFlow().getSDTOfFlowRole() == SDTOfFlowRole.BySpecDateField)
		{
			try
			{
				gwf.setSDTOfFlow(this.getHisWork().GetValStrByKey(this.getHisFlow().GetParaString("SDTOfFlowRole_DateField")));
				gwf.setRDTOfSetting(this.getHisWork().GetValStrByKey(this.getHisFlow().GetParaString("SDTOfFlowRole_StartDateField")));
			}
			catch (RuntimeException ex)
			{
		...
		{
			//获取流程的所有节点
			Nodes nds = new Nodes(this.getHisFlow().No);
			LocalDateTime sdtOfFlow = LocalDateTime.now();
			for (Node nd : nds)
			{
				...
				if (nd.getHisCHWay() == CHWay.ByTime && nd.GetParaInt("CHWayOfTimeRole") == 0)
				{ //按天、小时考核
				 //增加天数. 考虑到了节假日. 
				  //判断是修改了节点期限的天数
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		wl.setSDT("无");
		wl.setDTOfWarning(DataType.CurrentDataTimess);
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		this.rptGe.setFlowStartRDT(DataType.CurrentDataTimess);
		this.rptGe.setFlowEnderRDT(DataType.CurrentDataTimess);
	}
	...
	*/
	public final void DoCopyCurrentWorkDataToRpt()
	{
		...
		rptGe.setFlowEnderRDT(DataType.CurrentDataTimess);

		//设置当前的流程所有的用时.
		rptGe.setFlowDaySpan(DataType.GeTimeLimits(this.rptGe.GetValStringByKey(GERptAttr.FlowStartRDT), DataType.CurrentDataTime));
		...
		if (this.getHisFlow().getPTable().equals(this.getHisWork().EnMap.PhysicsTable))
		{
			rptGe.DirectUpdate();
		}
		else
	...
	*/
	...
	/** 
	 ...
	 @param msg
	*/
	public final void AddToTrack(ActionType at, GenerWorkerList gwl, String msg, long subTreadWorkID)
	{
		Track t = new Track();
		...
		switch (at)
		{
			...
			case TeampUp:
				...
				else
				{
					...
					{
						//获取审核组件信息
						String sql = "SELECT Msg From ND" + Integer.parseInt(this.getHisNode().getFK_Flow()) + "Track Where WorkID=" + t.getWorkID() + " AND FID=" + t.getFID() + " AND ActionType=" + ActionType.WorkCheck.getValue() + " AND NDFrom=" + this.getHisNode().getNodeID() + " AND EmpFrom='" + WebUser.No + "'";
						 t.setMsg(t.getMsg() + "WorkCheck@" + DBAccess.RunSQLReturnStringIsNull(sql, ""));

						//把审核组件的立场信息保存在track表中
						String checkTag = Dev2Interface.GetCheckTag(this.getHisNode().getFK_Flow(), this.getWorkID(), this.getHisNode().getNodeID(), WebUser.No);
	...
	*/
	...
	public final void AddToTrack(ActionType at, String toEmp, String toEmpName, int toNDid, String toNDName, String msg, Node ndFrom, String frmDBJson)
	{
...
//C# TO JAVA CONVERTER NOTE: Java does not support optional parameters. Overloaded method(s) are created above:
//ORIGINAL LINE: public void AddToTrack(ActionType at, string toEmp, string toEmpName, int toNDid, string toNDName, string msg, Node ndFrom, string frmDBJson = null, string tag = null)
	public final void AddToTrack(ActionType at, String toEmp, String toEmpName, int toNDid, String toNDName, String msg, Node ndFrom, String frmDBJson, String tag)
	{
		Track t = new Track();
		...
		this.getHisGenerWorkFlow().setSendDT(DataType.CurrentDataTime);
		this.getHisGenerWorkFlow().Update();

		GenerWorkerList gwl = new GenerWorkerList();
		int i = gwl.Retrieve(GenerWorkerListAttr.WorkID, this.getWorkID(), GenerWorkerListAttr.FK_Node, this.getHisNode().getNodeID(), GenerWorkerListAttr.FK_Emp, WebUser.No);
		if (i != 0)
		{
			gwl.setCDT(DataType.CurrentDataTimess);
			gwl.Update();
		}
	...
	*/
	private void DealAutoRunEnable()
	{
		...
		/*如果是自动运行就要设置自动运行参数.*/
		Object tempVar = this.getHisNode().getAutoRunParas().Clone();
		...
		//把约定的参数写入到引擎
		Dev2Interface.Flow_SetFlowTransferCustom(this.getHisFlow().No, this.getWorkID(), BP.WF.TransferCustomType.ByWorkerSet, strs);
	...
	*/
	private void CheckCompleteCondition()
	{
		...
		// 如果结束流程，就增加如下信息 翻译.
		this.getHisGenerWorkFlow().setSender(BP.Web.WebUser.No);
		this.getHisGenerWorkFlow().setSendDT(DataType.CurrentDataTime);

		this.rptGe.setFlowEnder(BP.Web.WebUser.No);
		this.rptGe.setFlowEnderRDT(DataType.CurrentDataTime);
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		try
		{
			...
			if (this.getHisNode().getHisToNodes().size() == 0 && this.getHisNode().getIsStartNode())
			{
				...
				String str2 = BP.WF.Glo.multilingual("工作已经成功处理(一个流程的工作)。 @查看<img src='./Img/Btn/PrintWorkRpt.gif' >", "WorkNode", "match_wf_completed_condition");
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	*/
	private void NodeSend_53_UnSameSheet_To_HeLiu(Node nd)
	{
		...
		{
			GenerWorkerList gwl = new GenerWorkerList(this.getHisWork().getFID(), nd.getNodeID(), WebUser.No);
			ActionType at = ActionType.SubThreadForward;
			this.AddToTrack(at, gwl, BP.WF.Glo.multilingual("子线程", "WorkNode", "sub_thread"), this.town.getHisWork().getOID());
		}
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		if (this.getHisNode().getTodolistModel() == WF.TodolistModel.QiangBan)
		{
			ps = new Paras();
...
///#warning 对于多个分合流点可能会有问题。
		...
		ps.Add("FID", this.getHisWork().getFID());
		BigDecimal numAll1 = (BigDecimal)DBAccess.RunSQLReturnValInt(ps);

		BigDecimal numPassed = gwf.GetParaInt("ThreadCount");

		BigDecimal passRate1 = numPassed.divide(numAll1.multiply(100));
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		for (MapDtl dtl : mydtls)
		{
			...
			geDtl.RefPK = String.valueOf(this.getHisWork().getFID()); // RefPK 就是当前子线程的FID.
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
			...
			if (dtl.IsEnableAthM == true)
			{
				...
				for (FrmAttachmentDB athDB : athDBs)
				{
					...
					athDB_N.UploadGUID = "";
					athDB_N.FID = this.getHisWork().getFID();
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		for (FrmAttachment ath : aths)
		{
			...
			for (FrmAttachmentDB athDB : athDBs)
			{
				...
				int num = athNDB.Retrieve(FrmAttachmentDBAttr.FK_MapData, "ND" + ndOfHeLiu.getNodeID(), FrmAttachmentDBAttr.RefPKVal, String.valueOf(this.getHisWork().getFID()), FrmAttachmentDBAttr.UploadGUID, athDB.UploadGUID);
				if (num > 0)
				{
					continue;
				}
				...
				FrmAttachmentDB athDB_N = new FrmAttachmentDB();
				athDB_N.Copy(athDB);
				athDB_N.FK_MapData = "ND" + ndOfHeLiu.getNodeID();
				athDB_N.RefPKVal = String.valueOf(this.getHisWork().getFID());
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		if (eleDBs.size() > 0)
		{
			...
			for (FrmEleDB eleDB : eleDBs)
			{
				...
				FrmEleDB eleDB_N = new FrmEleDB();
				eleDB_N.Copy(eleDB);
				eleDB_N.FK_MapData = "ND" + ndOfHeLiu.getNodeID();
				eleDB_N.MyPK = eleDB_N.MyPK.Replace("ND" + this.getHisNode().getNodeID(), "ND" + ndOfHeLiu.getNodeID());
				eleDB_N.RefPKVal = String.valueOf(this.getHisWork().getFID());
				eleDB_N.Save();
			}
		}
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		if (nodeid == 0)
		{
			switch (this.getHisNode().getHisRunModel())
			{
				...
				case FHL:
					...
					break;
				case SubThread:
					sql = "SELECT NDFrom FROM " + truckTable + " WHERE WorkID=" + this.getWorkID() + " AND NDTo=" + this.getHisNode().getNodeID() + " "
																				   + " AND ( ActionType=" + ActionType.SubThreadForward.getValue() + " OR  ActionType=" + ActionType.ForwardFL.getValue() + ")  ORDER BY RDT DESC";
					if (DBAccess.RunSQLReturnCOUNT(sql) == 0)
					{
						sql = "SELECT NDFrom FROM " + truckTable + " WHERE WorkID=" + this.getHisWork().getFID() + " AND NDTo=" + this.getHisNode().getNodeID() + " "
																				  + " AND (ActionType=" + ActionType.SubThreadForward.getValue() + " OR  ActionType=" + ActionType.ForwardFL.getValue() + ") ORDER BY RDT DESC";
					}
