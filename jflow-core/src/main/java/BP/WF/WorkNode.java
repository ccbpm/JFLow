			...
			if (!nd.getNodeFrmID().equals(mywork.NodeFrmID)) {
				...
				if (1 == 2) {
					...
					if (this.getHisWork().getEnMap().getPhysicsTable() == nd.getHisWork().getEnMap()
						...
						if (athDesc.size() > 0) {
							...
							if (athDBs.size() > 0) {
								athDBs.Delete(FrmAttachmentDBAttr.FK_MapData, "ND" + nd.getNodeID(),
										FrmAttachmentDBAttr.RefPKVal, this.getWorkID());
								...
								for (FrmAttachmentDB athDB : athDBs.ToJavaList()) {
									...
									// "_" + athDB_N.FK_MapData;
									// if (athDB.dbt
									// athDB_N.MyPK =
									// athDB_N.FK_FrmAttachment.replace("ND" +
									// this.HisNode.NodeID, "ND" + nd.NodeID) +
									// "_" + this.WorkID;

									athDB_N.setMyPK(DBAccess.GenerGUID()); // athDB_N.FK_FrmAttachment.replace("ND"
																			// +
																			// this.HisNode.NodeID,
																			// "ND"
																			// +
						...
						String recDtlLog = "@记录测试明细表Copy过程,从节点ID:" + this.getHisNode().getNodeID() + " WorkID:"
								+ this.getWorkID() + ", 到节点ID=" + nd.getNodeID();
						if (dtls.size() > 0) {
							...
							for (MapDtl dtl : dtls.ToJavaList()) {
								...
								DBAccess.RunSQL("DELETE FROM " + toDtl.getPTable() + " WHERE RefPK=" + dbStr + "RefPK",
										"RefPK", (new Long(this.getWorkID())).toString());
