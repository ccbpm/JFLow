...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	*/
	public final String DoOrderReturn()
	{
		...
		// 加入track.
		this.AddToTrack(ActionType.Return, returnToEmp.No, returnToEmp.Name, this.ReturnToNode.getNodeID(), this.ReturnToNode.getName(), Msg);
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		//退回后事件
		String text = this.HisNode.getHisFlow().DoFlowEventEntity(EventListOfNode.ReturnAfter, this.HisNode, work, atPara);
		...
		{
			return "工作已经被您退回到(" + this.ReturnToNode.getName() + "),退回给(" + returnToEmp.No + "," + returnToEmp.Name + ").\n\r" + text;
		}
	}
	/** 
	 要退回到父流程上去
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	*/
	public final String DoIt()
	{
		...
		{
			BP.DA.DBAccess.RunSQL("DELETE FROM ND" + Integer.parseInt(this.HisNode.getFK_Flow()) + "Track WHERE WorkID = " + this.WorkID + " AND ActionType = " + ActionType.WorkCheck.getValue() + " AND NDFrom = " + this.HisNode.getNodeID() + " AND NDTo = " + this.HisNode.getNodeID() + " AND (Msg = '' OR Msg IS NULL)");
		}
	...
	*/
	private String ExeReturn5_2()
	{
		...
		int i = currWorker.Retrieve(GenerWorkerListAttr.FK_Emp, WebUser.No, GenerWorkerListAttr.WorkID, this.WorkID, GenerWorkerListAttr.FK_Node, this.HisNode.getNodeID());
	...
	*/
	private String ExeReturn1_1()
	{
		...
		LocalDateTime dtOfShould = LocalDateTime.MIN;
			...
			//增加天数. 考虑到了节假日.             
			dtOfShould = Glo.AddDayHoursSpan(LocalDateTime.now(), this.ReturnToNode.getTimeLimit(), this.ReturnToNode.getTimeLimitHH(), this.ReturnToNode.getTimeLimitMM(), this.ReturnToNode.getTWay());
		...
		gwf.setSender(WebUser.No + "," + WebUser.Name);
		...
		for (GenerWorkerList item : gwls)
		{
			...
			item.setIsRead(false);
			item.setSDT(sdt);
			item.setRDT(DataType.CurrentDataTime);
			item.setSender(WebUser.Name + "," + WebUser.No);
			item.Update();
		}
		...
		if (this.HisNode.getTodolistModel() == TodolistModel.Order || this.HisNode.getTodolistModel() == TodolistModel.Sharing || this.HisNode.getTodolistModel() == TodolistModel.TeamupGroupLeader || this.HisNode.getTodolistModel() == TodolistModel.Teamup)
		{
			...
			String sql = "UPDATE WF_GenerWorkerlist SET IsPass=1000 WHERE FK_Node=" + this.HisNode.getNodeID() + " AND WorkID=" + this.WorkID + " AND FK_Emp='" + WebUser.No + "'";
	...
	*/
	...
	{
		String filePath = BP.Sys.SystemConfig.PathOfDataUser + "\\ReturnLog\\" + this.HisNode.getFK_Flow() + "\\";
	...
	{
		/*开始遍历到达的节点集合*/
		for (Node nd : fromND.getHisToNodes())
		{
			...
			if (nd.getIsFL())
			{
				...
				for (GenerWorkerList wl : wls)
				{
					...
					infoLog += "\r\n*****************************************************************************************";
					infoLog += "\r\n节点ID:" + subNd.getNodeID() + "  工作名称:" + subWK.EnDesc;
					infoLog += "\r\n处理人:" + subWK.getRec() + " , " + wk.getRecOfEmp().Name;
			...
			{
				infoLog += "\r\n*****************************************************************************************";
				infoLog += "\r\n节点ID:" + wk.getNodeID() + "  工作名称:" + wk.EnDesc;
				infoLog += "\r\n处理人:" + wk.getRec() + " , " + wk.getRecOfEmp().Name;
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	private WorkNode DoReturnSubFlow(int backtoNodeID, String msg, boolean isHiden)
	{
		...
		wl.setSDT(dtNew.toString(DataType.SysDataTimeFormat)); // DataType.CurrentDataTime;
		...
		try
		{
			rw.MyPK = rw.getReturnToNode() + "_" + rw.getWorkID() + "_" + LocalDateTime.now().toString("yyyyMMddhhmmss");
			rw.Insert();
		}
		catch (java.lang.Exception e)
		{
			rw.MyPK = rw.getReturnToNode() + "_" + rw.getWorkID() + "_" + BP.DA.DBAccess.GenerOID();
			rw.Insert();
		}

		// 加入track.
		this.AddToTrack(ActionType.Return, FK_Emp, emp.Name, backtoNodeID, nd.getName(), msg);
		...
		if (Glo.getIsEnableSysMessage())
		{
			...
			String title = String.format("工作退回：流程:%1$s.工作:%2$s,退回人:%3$s,需您处理", wn.getHisNode().getFlowName(), wn.getHisNode().getName(), WebUser.Name);
