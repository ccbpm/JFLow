...
public class Dept extends EntityTree {
    ...
    protected boolean beforeDelete() throws Exception {
        ...
        ens.Retrieve(DeptEmpAttr.FK_Dept, this.getNo());
        if (ens.size() > 0) {
            throw new RuntimeException("err@删除部门错误，该部门下有人员。");
        }

        DeptEmpStations ensD = new DeptEmpStations();
        ensD.Retrieve(DeptEmpAttr.FK_Dept, this.getNo());
        if (ensD.size() > 0) {
    ...
    protected boolean beforeInsert() throws Exception {
        if (bp.da.DataType.IsNullOrEmpty(this.getNo()) == true) {
            this.setNo(bp.da.DBAccess.GenerGUID(5, "Port_Dept", "No"));
        }

        if (bp.da.DataType.IsNullOrEmpty(this.getOrgNo()) == true) {
    ...
    public final void GenerNameOfPath() throws Exception {
        ...
        //根目录不再处理.
        if (this.getIsRoot() == true || this.getNo().equals("100") == true) {
        ...
        while (true) {
            if (dept.getIsRoot() || dept.getNo().equals("100")) {
                break;
            }
