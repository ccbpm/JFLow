...
public class MapAttr extends EntityMyPK {
	...
	protected boolean beforeInsert() throws Exception {
		...
		if (this.getIdx() == 0) {
			this.setIdx(DBAccess.RunSQLReturnValInt(
					String.format("SELECT %1$s FROM Sys_MapAttr WHERE FK_MapData='" + this.getFK_MapData() + "'",
							SqlBuilder.GetIsNullInSQL("MAX(Idx)", "0")))
					+ 1);
		}
		this.setMyPK(this.getFK_MapData() + "_" + this.getKeyOfEn());
	...
	protected boolean beforeDelete() throws Exception {
		String sqls = "DELETE FROM Sys_MapExt WHERE (AttrOfOper='" + this.getKeyOfEn() + "' OR AttrsOfActive='"
				+ this.getKeyOfEn() + "' ) AND (FK_MapData='" + this.getFK_MapData() + "')";
		// 删除权限管理字段.
		...
		if (this.getUIContralType() == UIContralType.DDL && this.getLGType() == FieldTypeS.Normal) {
			sqls += "@DELETE FROM Sys_MapAttr WHERE KeyOfEn='" + this.getKeyOfEn() + "T' AND FK_MapData='"
					+ this.getFK_MapData() + "'";
		}
