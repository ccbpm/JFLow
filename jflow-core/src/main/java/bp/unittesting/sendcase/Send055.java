...
public class Send055 extends TestBase
{
	/** 
	 客户参与流程
	*/
	...
	*/
	public final void Step2_1() throws Exception
	{
		...
		//获得此人的 055 的待办工作.
		dt = bp.wf.Dev2Interface.DB_GenerEmpWorksOfDataTable(WebUser.getNo(), WFState.Runing, "055");
		if (dt.Rows.size() == 0)
		{
	...
	*/
	public final void Step2_2() throws Exception
	{
		...
		if (!dt.Rows.get(0).getValue("FuWuQi").toString().equals("100"))
		{
			throw new RuntimeException("没有存储到指定的位置.");
		}

		if (!dt.Rows.get(0).getValue("ShuMaXiangJi").toString().equals("30"))
		{
			throw new RuntimeException("没有存储到指定的位置.");
		}
	...
	*/
	public final void Step3() throws Exception
	{
		...
		if (!dt.Rows.get(0).getValue(GERptAttr.FlowEndNode).toString().equals("599"))
		{
