...
/** 
 发送多人测试
*/
public class SendCaseToEms extends TestCaseTemplete
{
	/** 
	 发送多人测试
	*/
	...
	*/
	...
	public void Do() throws Exception
	{
		...
		objs = bp.wf.Dev2Interface.Node_SendWork(this.fk_flow, this.workid, null, null, 0, toEmps);
			...
			///检查发送变量是否正确?
		if (!objs.getVarAcceptersID().equals(toEmps))
		{
		...
		if (!gwf.getFK_Flow().equals(fk_flow))
		{
		...
		if (!gwf.getStarter().equals(userNo))
		{
			throw new RuntimeException("@当前 Starter 错误应当是:" + userNo + ",现在是:" + gwf.getStarter());
		}

		if (!gwf.getStarterName().equals(WebUser.getName()))
		{
		...
		for (GenerWorkerList wl : wls.ToJavaList())
		{
			...
			{
				if (!wl.getFK_Dept().equals(WebUser.getFK_Dept()))
				{
					throw new RuntimeException("@部门错误,应该是" + WebUser.getFK_Dept() + ",现在是:" + wl.getFK_Dept());
				}

				if (!wl.getFK_Flow().equals(fk_flow))
				{
				...
				//if (wl.Sender != WebUser.getNo() + ",张海成")
				//    throw new Exception("@ Sender 错误,应该是" + WebUser.getNo() + ",张海成 ,现在是:" + wl.Sender);
			}
			...
			if (wl.getFK_Emp().equals("zhoushengyu") || wl.getFK_Emp().equals("zhangyifan"))
			{
				...
				if (!wl.getFK_Dept().equals(emp.getFK_Dept()))
				{
