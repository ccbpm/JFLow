...
public class Node extends Entity {
	...
	public final DirCondModel getCondModel() throws Exception {
		return DirCondModel.forValue(this.GetValIntByKey(NodeAttr.CondModel));
	}
	 ...
	 * 待办删除规则
	 */
	public final int getGenerWorkerListDelRole() throws Exception {
		return this.GetParaInt("GenerWorkerListDelRole", 0);
	}

	public final void setGenerWorkerListDelRole(int value) throws Exception {
	...
	public final bp.wf.template.NodeSimples getHisToNodeSimples() throws Exception {
		...
		if (obj == null) {
			...
			if (this.getHisToNDNum() > 1) {
				...
				// @101@102@103.
				String[] nds = this.getHisToNDs().split("[@]", -1);
	...
	public final Work getHisWork() throws Exception {
		...
		bp.en.Map ma = (bp.en.Map) wk.getEnMap();
	...
	public final Works getHisWorks() throws Exception {
		Works obj = this.getHisWork().getGetNewEntities() instanceof Works
				? (Works) this.getHisWork().getGetNewEntities()
				: null;
		return obj;
	}
	...
	public final PushMsgs getHisPushMsgs() throws Exception {
		...
		if (obj == null) {
			Entities ens = this.GetEntitiesAttrFromAutoNumCash(new PushMsgs(), PushMsgAttr.FK_Node, this.getNodeID());
	...
	public final FrmEvents getFrmEvents() throws Exception {
		Entities ens = this.GetEntitiesAttrFromAutoNumCash(new FrmEvents(), FrmEventAttr.FK_Node, this.getNodeID());
	...
	protected boolean beforeUpdate() throws Exception {
		...
		if (this.getIsStartNode()) {
			...
			if (this.getHisDeliveryWay() != oldN.getHisDeliveryWay()) {
				// 清空WF_Emp中的StartFlow
				DBAccess.RunSQL("UPDATE  WF_Emp Set StartFlows =''");
			}
		}
		...
		/// 如果是数据合并模式，就要检查节点中是否有子线程，如果有子线程就需要单独的表.
	...
	public final int getWarningDay() throws Exception {
		int i = (int) this.GetValFloatByKey(NodeAttr.WarningDay);
		if (i == 0) {
			return 1;
		}
		return i;
	...
	public final boolean getReturnCHEnable() throws Exception {
		return this.GetValBooleanByKey(NodeAttr.ReturnCHEnable);
	}

	public final void setReturnCHEnable(boolean value) throws Exception {
	 ...
	 * 是否是自定义的url,处理批处理.
	 */
	public final boolean getBatchParasIsSelfUrl() throws Exception {
	...
	public final void setCHRole(int value) throws Exception {
		this.SetValByKey(BtnAttr.CHRole, value);
	}

	///

	/// 公共方法 (用户执行动作之后,所要做的工作)
	/**
	 * 用户执行动作之后,所要做的工作
	 * 
	 * @return 返回消息,运行的消息
	...
	public final float getFwcW() throws Exception {
	...
	public final boolean getIsDeleteModel() throws Exception {
		return this.GetValBooleanByKey(NodeAttr.IsDeleteModel);
	}
	...
	public Map getEnMap() throws Exception {
		...
		// map.AddTBString(NodeAttr.FK_FlowSort, null, "FK_FlowSort", false, true, 0, 4,
		// 10);
		// map.AddTBString(NodeAttr.FK_FlowSortT, null, "FK_FlowSortT", false, true, 0,
		// 100, 10);
		...
		map.AddTBFloat(NodeAttr.TimeLimit, 2, "限期(天)", true, false); // "限期(天)".
																		// map.AddTBFloat(NodeAttr.TSpanHour, 0, "小时",
																		// true, false); //"限期(分钟)".
		...
		map.AddBoolean(NodeAttr.IsDeleteModel, false, "是否删除中间节点信息(不是原来返回时有效)", true, true);
		...
		// map.AddTBString(NodeAttr.HisEmps, null, "HisEmps", false, false, 0, 3000,
		// 10);
		...
		// map.AddTBString(FrmEventAttr.MailTitle, null, "邮件标题模版", true, false, 0, 200,
		// 20, true);
		...
		// map.AddBoolean(FrmEventAttr.MobilePushEnable, true, "是否推送到手机、pad端。", true,
		//// true, true);
	...
	protected boolean beforeDelete() throws Exception {
		...
		// 判断是否可以被删除.
		num = DBAccess.RunSQLReturnValInt(
				"SELECT COUNT(*) FROM WF_GenerWorkerlist WHERE IsPass=1 and FK_Node=" + this.getNodeID());
		if (num != 0) {
	...
	public boolean getIsSendDraftSubFlow() throws Exception {
