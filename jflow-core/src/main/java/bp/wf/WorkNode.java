...
public class WorkNode {
	...
	private GenerWorkerLists InitWorkerLists(WorkNode town, DataTable dt, long fid) throws Exception {
		...
		default:
			...
			// this.HisGenerWorkFlow.SetPara("CH" + this.town.HisNode.NodeID,
			// this.HisGenerWorkFlow.SDTOfNode);
			// this.HisGenerWorkFlow.SDTOfFlow =
			// dtOfFlow.ToString(DataType.SysDataTimeFormat);
			// this.HisGenerWorkFlow.SDTOfFlowWarning =
			// dtOfFlowWarning.ToString(DataType.SysDataTimeFormat);
		...
		if (dt.Rows.size() == 1) {
			...
			for (DataRow dr : dt.Rows) {
				...
				/// 增加存储的优先级，防止数据库根据FK_Emp的人员编码自动排序，导致查询的结果和接收人的先后顺序不一致
				idx++;
				wl.setIdx(idx);
				...
				if (town.getHisNode().getHisCHWay() == CHWay.None) {
					...
					wl.setSDT(DataType.dateToStr(dtOfShould, DataType.getSysDataTimeFormat() + ":ss"));
				}

				wl.setDTOfWarning(DataType.dateToStr(dtOfWarning, DataType.getSysDataTimeFormat()));
	...
	public final Node NodeSend_GenerNextStepNode() throws NumberFormatException, Exception {
		...
		// 判断是否有用户选择的节点.
		if (this.getHisNode().getCondModel() == DirCondModel.ByUserSelected) {
			// 获取用户选择的节点.
	 ...
	 * @throws NumberFormatException
	 */
	public final Node NodeSend_GenerNextStepNode_Ext1() throws NumberFormatException, Exception {
		// 如果要是跳转到的节点，自动跳转规则规则就会失效。
		if (this.JumpToNode != null) {
			return this.JumpToNode;
		}
			...
			if (dt == null || dt.Rows.size() == 0) {
				if (nd.getHisWhenNoWorker() == true) {
					this.AddToTrack(ActionType.Skip, this.getExecer(), this.getExecerName(), nd.getNodeID(),
							nd.getName(), bp.wf.Glo.multilingual("自动跳转(启用跳转规则,当没有找到处理人时)", "WorkNode",
			...
			if (nd.getAutoJumpRole0()) {
					...
					if (dr.getValue(0).toString().equals(this.getHisGenerWorkFlow().getStarter())) {
						...
						for (Attr attr : attrs) {
							if (attr.getUIIsReadonly() && attr.getUIVisible() == true
								&&DataType.IsNullOrEmpty(attr.getDefaultValOfReal())==false) {
				...
				if (isHave == true) {
					...
					this.AddToTrack(ActionType.Skip, Executor, ExecutorName, nd.getNodeID(), nd.getName(),
							bp.wf.Glo.multilingual("自动跳转(发起人就是处理人时)", "WorkNode", "system_error_jump_automatically_2",
									new String[0]),
							getNdFrom());
			...
			if (nd.getAutoJumpRole2()) {
				...
				if (isHave == true) {
					...
					if (!nd.getNodeFrmID().equals(wk.NodeFrmID)) {
						if (wk.getEnMap().getPhysicsTable() != this.rptGe.getEnMap().getPhysicsTable()) {
							wk.Copy(this.rptGe);
						}
	...
	private void CC(Node node) throws Exception {
		...
		if (this.getHisNode().getHisCCRole() == CCRole.HandCC) {
			...
			if (cclist.size() > 0) {
				...
				String mytemp = "";
				if (pushMsg != null) {
					String title = String.format("工作抄送:%1$s.工作:%2$s,发送人:%3$s,需您查阅", node.getFlowName(), node.getName(),
							WebUser.getName());
				...
				String atParas = "@FK_Flow=" + node.getFK_Flow() + "@WorkID=" + this.getWorkID() + "@NodeID="
						+ node.getNodeID() + "@FK_Node=" + node.getNodeID();
				for (CCList cc : cclist.ToJavaList()) {
					ccMsg1 += "(" + cc.getCCTo() + " - " + cc.getCCToName() + ");";
					if (pushMsg != null) // 抄送的WorkID=0,目的不让其删除.
					{
						bp.wf.Dev2Interface.Port_SendMessage(cc.getCCTo(), mytemp, title, EventListNode.CCAfter,
								"WKAlt" + node.getNodeID() + "_" + this.getWorkID(), WebUser.getNo(), "",
								pushMsg.getSMSPushModel(), 0, null, atParas);
					}
				}
			}
		}
	...
	private Node NodeSend_GenerNextStepNode_Ext(Node currNode) throws Exception {
		...
		for (Direction dir : dirs.ToJavaList()) {
			...
			conds.Retrieve(CondAttr.FK_Node, currNode.getNodeID(), CondAttr.ToNodeID, dir.getToNode(),
	...
	public final Nodes Func_GenerNextStepNodes() throws Exception {
		...
		for (Node nd : toNodes.ToJavaList()) {
			Conds dcs = new Conds();
			dcs.Retrieve(CondAttr.FK_Node, this.getHisNode().getNodeID(), CondAttr.ToNodeID, nd.getNodeID(),
					CondAttr.CondType, CondType.Dir.getValue(), CondAttr.Idx);
		...
		/// 走到最后，发现一个条件都不符合，就找没有设置方向条件的节点. （@杜翻译）
		if (myNodes.size() == 0) {
			/* 如果没有找到其他节点，就找没有设置方向条件的节点. */
			for (Node nd : toNodes.ToJavaList()) {
				Conds conds = new Conds();
				int i = conds.Retrieve(CondAttr.FK_Node, nd.getNodeID(), CondAttr.CondType, 2);
				if (i == 0) {
					continue;
				}
	...
	private String Func_DoSetThisWorkOver() throws Exception {
		...
		/// 2014-08-02 删除了其他人员的待办，增加了 IsPass=0 参数.
	...
	private void NodeSend_11(Node toND) throws Exception {
		...
		if (town.getHisNode().getHuiQianRole() == HuiQianRole.TeamupGroupLeader
			...
			if (town.getHisNode().getFWCOrderModel() == 1) {
				...
				for (GenerWorkerList gwl : current_gwls.ToJavaList()) {
					...
					bp.wf.Dev2Interface.WriteTrack(this.getHisFlow().getNo(), town.getHisNode().getNodeID(),
							...
							ActionType.WorkCheck, null, null, null,null, gwl.getFK_Emp(), gwl.getFK_EmpText(),
							gwl.getFK_Emp(), gwl.getFK_EmpText(), rdt,null);
				}
			}
		}
		...
		if (toND.getIsGuestNode() == true) {
			...
			// @发送给如下{0}位处理人,{1}.
			// textInfo =
			// Glo.Multilingual("WorkNode","zp100",this.HisRememberMe.NumOfObjs.ToString(),
			// this.HisGenerWorkFlow.GuestName);
		...
		if (this.HisWorkerLists.size() >= 2 && this.getHisNode().getIsTask()) {
			...
			this.addMsg(SendReturnMsgFlag.AllotTask, null,
					"<a href='./WorkOpt/AllotTask.htm?WorkID=" + this.getWorkID() + "&FK_Node=" + toND.getNodeID()
							+ "&FK_Flow=" + toND.getFK_Flow() + "'  target=_self><img src='" + SystemConfig.getHostURLOfBS()
				...
				if (this.getHisNode().getIsStartNode()) {
					this.addMsg(SendReturnMsgFlag.ToEmpExt, null,
							"@<a href='./WorkOpt/UnSend.htm?DoType=UnSend&UserNo=" + WebUser.getNo() + "&SID="
									...
									+ toND.getFK_Flow() + "' ><img src='" + SystemConfig.getHostURLOfBS()
		...
		// this.addMsg(SendReturnMsgFlag.WorkStartNode,
		// Glo.Multilingual("WorkNode","WorkStartNode",toND.getName()),
		// "WorkStartNode1");
			...
			for (GenerWorkerList wl : current_gwls.ToJavaList()) {
				...
				if (gwf.getIsExits() == false) {
					...
					if (DataType.IsNullOrEmpty(this.getHisFlow().getBuessFields()) == false) {
						// 存储到表里atPara
						// @BuessFields=电话^Tel^18992323232;地址^Addr^山东济南;
	...
	private void NodeSend_24_SameSheet(Node toNode) throws Exception {
		...
		for (GenerWorkerList wl : current_gwls.ToJavaList()) {
			...
			if (IsHaveFH) {
				...
				if (this.IsHaveSubThreadGroupMark == true) {
					...
					if (dt.Rows.size() == 0) {
						mywk.setOID(DBAccess.GenerOID("WorkID")); // DBAccess.GenerOID();
					} else {
			...
			if (isHaveFLDtl == true) {
				...
				for (MapDtl dtl : this.getHisWork().getHisMapDtls().ToJavaList()) {
					...
					for (GEDtl gedtl : gedtls.ToJavaList()) {
						...
						if (DataType.IsNullOrEmpty(dtl.getSubThreadWorker()) == false) {
							String groupMarkDtl = gedtl.GetValStringByKey(dtl.getSubThreadWorker());
			...
			/// 处理烟台需求, 需要合流节点有一个明细表，根据明细表启动子线程任务,并要求把明细表的一行数据copy到下一个子线程的主表上去。
		...
		if (this.getHisNode().getHisCancelRole() != CancelRole.None){
			if (this.getHisNode().getIsStartNode()) {
				...
				this.addMsg("UnDo", "@<a href='./WorkOpt/UnSend.htm?DoType=UnSend&UserNo=" + WebUser.getNo() + "&SID="
	...
	private void NodeSend_53_SameSheet_To_HeLiu(Node toNode) throws Exception {
		...
		if (fid == 0) {
			...
			String strs = DBAccess.RunSQLReturnStringIsNull(
		...
		// 分流节点和子线程的节点绑定的表单相同
		if (toNode.getHisFormType() == NodeFormType.RefOneFrmTree
				&& toNode.getNodeFrmID().equals(this.getHisNode().getNodeFrmID()) == true) {
			isCopyData = false;
		}
		if (isCopyData == true) {
		...
		this.addMsg("InfoToHeLiu", bp.wf.Glo.multilingual("@流程已经运行到合流节点[{0}]. @您的工作已经发送给如下人员[{1}]. @您是第{2}个到达此节点的处理人.",
				"WorkNode", "first_node_person", toNode.getName(), toEmpsStr, String.valueOf(count)));

		/// 处理国机的需求, 把最后一个子线程的主表数据同步到合流节点的Rpt里面去.(不是很合理) 2015.12.30
	 ...
	 * @throws NumberFormatException 
	*/
	private void NodeSend_Send_5_5() throws NumberFormatException, Exception
	{
...
*/
	...
	public final void CopyData(Work toWK, Node toND, boolean isSamePTable) throws Exception
	{
		...
		if (dtls.size() > 0)
		{
			...
			recDtlLog += bp.wf.Glo.multilingual("@到节点明细表数量是{0}个", "WorkNode", "count_of_detail_table", String.valueOf(dtls.size()));
			...
			for (MapDtl dtl : dtls.ToJavaList())
			{
				recDtlLog += bp.wf.Glo.multilingual("@进入循环开始执行明细表({0})copy:", "WorkNode", "start_copy_detail_table", dtl.getNo());
				...
				recDtlLog += bp.wf.Glo.multilingual("@从明细表({0})查询数据一共{1}条.", "WorkNode", "log_detail_table_1", dtl.getNo(), String.valueOf(gedtls.size()));
				...
				recDtlLog += bp.wf.Glo.multilingual("@数据删除到达明细表:{0},并开始遍历明细表,执行一行行的copy.", "WorkNode", "log_detail_table_2", dtl.getNo());

				if (DBAccess.IsExitsObject(toDtl.getPTable()))
				{
					...
					if (toDtl.getIsEnableAthM())
					{
						...
						if (athDBs.size() > 0)
						{
							...
							for (FrmAttachmentDB athDB : athDBs.ToJavaList())
							{
								...
								athDB_N.Copy(athDB);
								athDB_N.setFK_MapData(toDtl.getNo());
								athDB_N.setMyPK(athDB.getMyPK() + "_" + dtCopy.getOID() + "_" + String.valueOf(i));
								athDB_N.setFK_FrmAttachment(athDB_N.getFK_FrmAttachment().replace("ND" + this.getHisNode().getNodeID(), "ND" + toND.getNodeID()));
								athDB_N.setRefPKVal(String.valueOf(dtCopy.getOID()));
								try
								{
...
///#warning 记录日志.
	...
	public final SendReturnObjs NodeSend(Node jumpToNode, String jumpToEmp) throws Exception {
		SendReturnObjs sendObj = NodeSend(jumpToNode, jumpToEmp,false);
		return sendObj;
	}
	...
	public final void CheckFrm1ToN() throws Exception {
		...
		for (FrmNode fn : frmNDs.ToJavaList()) {
			...
			case PWorkID:
				if (this.rptGe == null) {
					this.rptGe = new GERpt("ND" + Integer.parseInt(this.getHisFlow().getNo()) + "Rpt",
							this.getWorkID());
				}
			...
			for (GenerWorkerList item : current_gwls.ToJavaList()) {
				...
				// if (attr.getDefaultValOfReal() == "@WebUser.FK_Dept")
				// {
				...
				// if (attr.getDefaultValOfReal() == "@WebUser.FK_DeptName")
				// {
				// Emp emp = new Emp(item.FK_Emp);
				// geEn.SetValByKey(attr.getKey(), emp.FK_DeptText);
				// continue;
				// }
				// }
	...
	public final void CheckFrmHuiZongToDtl() throws Exception {
		...
		for (FrmNode fn : frmNDs.ToJavaList()) {
			...
			dtlEn.setOID((int) this.getWorkID());
			dtlEn.setRDT(DataType.getCurrentDataTime());
	...
	public final boolean CheckFrmIsNotNull() throws Exception {
		...
		if (this.getHisNode().getHisFormType() == NodeFormType.FreeForm
			...
			/// 检查附件个数的完整性. - 该部分代码稳定后，移动到独立表单的检查上去。
			for (FrmAttachment ath : this.getHisWork().getHisFrmAttachments().ToJavaList()) {
				...
				if (ath.getReadRole() != 0) {
					...
					if (ids.contains("ALL") == false) {
						...
						// string sql = "SELECT MyPK,FileName FROM
						// Sys_FrmAttachmentDB WHERE RefPKVal=" + this.WorkID +
						// " AND FK_FrmAttachment='" + ath.MyPK + "' AND Rec!='"
						// + WebUser.getNo() + "'";
						// DataTable dt = DBAccess.RunSQLReturnTable(sql);
			...
			/// 检查图片附件的必填，added by liuxc,2016-11-1
	...
	public final boolean DealTeamUpNode() throws Exception {
		...
		for (GenerWorkerList gwl : gwls.ToJavaList()) {
			...
			// 处理会签问题
			this.addMsg(SendReturnMsgFlag.OverCurr, bp.wf.Glo.multilingual("@您已经完成签完工作. 当前未处理工作的人还有:{0}.", "WorkNode",
	...
	public final boolean DealTeamupGroupLeader() throws Exception {
		...
		if (this.getHisNode().getTeamLeaderConfirmRole() == TeamLeaderConfirmRole.BySQL) {
			...
			// 把当前的待办设置已办
			for (GenerWorkerList gwl : gwls.ToJavaList()) {
				if ((gwl.getIsPassInt() == 0 || gwl.getIsPassInt() == 90)
						&& (!gwl.getFK_Emp().equals(WebUser.getNo()))) {
					todoEmps += bp.wf.Glo.DealUserInfoShowModel(gwl.getFK_Emp(), gwl.getFK_EmpText()) + " ";
				}
				...
				this.AddToTrack(ActionType.TeampUp, gwl.getFK_Emp(), todoEmps, this.getHisNode().getNodeID(),
						this.getHisNode().getName(), "多人处理规则：协作组长模式");
			}
			this.addMsg(SendReturnMsgFlag.CondInfo, bp.wf.Glo.multilingual("@当前工作未处理的人还有: {0},所以不能发送到下一步.", "WorkNode",
					"you_have_finished_1", todoEmps), null, SendReturnMsgType.Info);
			return true;
		}
		...
		for (GenerWorkerList gwl : gwls.ToJavaList()) {
			if (gwl.getIsPassInt() > 10) {
				...
				if (this.getHisGenerWorkFlow().getEmps()
						.contains("@" + WebUser.getNo() + "," + WebUser.getName() + "@") == false
						|| this.getHisGenerWorkFlow().getEmps().contains("@" + WebUser.getNo() + "@") == false) {
					this.getHisGenerWorkFlow().setEmps(
							this.getHisGenerWorkFlow().getEmps() + WebUser.getNo() + "," + WebUser.getName() + "@");
				}
	...
	private void CheckBlockModel() throws Exception {
		...
		if (this.getHisNode().getBlockModel() == BlockModel.ByExp) {
			...
			if (row.containsKey(key) == false) {
				...
				valPara = row.GetValStrByKey(key).toString().trim();
			}
			...
			for (String str : strs) {
				...
				if (this.getHisNode().getHisRunModel() == RunModel.SubThread) {
					...
					if (gwfs.size() != 0) {
						...
						gwls.Retrieve(GenerWorkerListAttr.FID, gwf.getWorkID(), GenerWorkerListAttr.FK_Node,
								subFlowNode, GenerWorkerListAttr.IsPass, 1);
						if (gwls.size() != 0) {
							continue;
						}
					...
					if (gwfs.size() != 0) {
						...
						String sql = "";
						if (gwf.getFID() == 0) {
							sql = "SELECT count(*) as Num FROM ND" + Integer.parseInt(gwf.getFK_Flow())
									+ "Track WHERE WorkID=" + gwf.getWorkID() + " AND (NDFrom=" + subFlowNode
									+ " or NDTo=" + subFlowNode + " )";
						} else {
							sql = "SELECT count(*) as Num FROM ND" + Integer.parseInt(gwf.getFK_Flow())
									+ "Track WHERE FID=" + gwf.getWorkID() + " AND (NDFrom=" + subFlowNode + " or NDTo="
									+ subFlowNode + " )";
						}
						...
						// 做第2次判断.
						if (gwf.getFID() == 0) {
							sql = "SELECT count(*) as Num FROM WF_GenerWorkerList WHERE WorkID=" + gwf.getWorkID()
									+ " AND FK_Node=" + subFlowNode;
						} else {
							sql = "SELECT count(*) as Num FROM WF_GenerWorkerList  WHERE WorkID=" + gwf.getFID()
									+ " AND FK_Node=" + subFlowNode;
						}
		...
		if (this.getHisNode().getBlockModel() == BlockModel.SameLevelSubFlow) {
			...
			for (String str : strs) {
				...
				// qo.addAnd();
				// qo.AddWhere(GenerWorkFlowAttr.PNodeID,
				// this.HisGenerWorkFlow.PNodeID);
				qo.addAnd();
	...
	private SendReturnObjs NodeSendToYGFlow(Node node, String toEmpIDs) throws Exception {
		...
		if (DataType.IsNullOrEmpty(toEmpIDs)) {
			...
			if (node.getHisDeliveryWay() == DeliveryWay.ByTeamOnly) {
				sql = "SELECT A.FK_Emp No FROM Port_TeamEmp A, WF_NodeTeam B WHERE A.FK_Team=B.FK_Team AND B.FK_Node="
				...
				if (dt.Rows.size() == 0) {
					...
					para2[3] = ps.getSQLNoPara();
					throw new RuntimeException(
							bp.wf.Glo.multilingual("@节点访问规则{0}错误:节点({1},{2}), 仅按用户组计算，没有找到人员:SQL={3}.", "WorkNode",
									"error_in_access_rules_setting", para2));
				}
			}
			...
			if (node.getHisDeliveryWay() == DeliveryWay.ByTeamDeptOnly) {
				sql = "SELECT A.FK_Emp No FROM Port_TeamEmp A, WF_NodeTeam B, Port_Emp C WHERE A.FK_Emp=C.No AND A.FK_Team=B.FK_Team AND B.FK_Node="
						+ dbStr + "FK_Node AND C.FK_Dept=" + dbStr + "FK_Dept ORDER BY A.FK_Emp";
				ps = new Paras();
				ps.Add("FK_Node", node.getNodeID());
			...
			if (node.getHisDeliveryWay() == DeliveryWay.ByTeamOrgOnly) {
				sql = "SELECT A.FK_Emp No FROM Port_TeamEmp A, WF_NodeTeam B, Port_Emp C WHERE A.FK_Emp=C.No AND A.FK_Team=B.FK_Team AND B.FK_Node="
						+ dbStr + "FK_Node AND C.OrgNo=" + dbStr + "OrgNo ORDER BY A.FK_Emp";
				ps = new Paras();
				ps.Add("FK_Node", node.getNodeID());
		...
		// 设置消息.
		this.addMsg("Msg1", bp.wf.Glo.multilingual("子流程({0})已经启动,发送给({1})处理人.", "WorkNode", "sub_wf_started",
				node.getFlowName(), toEmpIDs));
	...
	public final SendReturnObjs NodeSend(Node jumpToNode, String jumpToEmp,boolean isReturnNode) throws Exception {
		...
		// 第1: 检查是否可以处理当前的工作.
		if (this.getHisNode().getIsStartNode() == false
				&& this.getHisGenerWorkFlow().getTodoEmps().contains(WebUser.getNo() + ",") == false) {
		...
		try {
			...
			if (this.getHisGenerWorkFlow().getWFState() == WFState.ReturnSta  ) {
				...
				/// 当前节点是分流节点但是是子线程退回的节点,需要直接发送给子线程
				if ((this.getHisNode().getHisRunModel() == RunModel.FL
						|| this.getHisNode().getHisRunModel() == RunModel.FHL)
						&& this.getHisGenerWorkFlow().getFID() != 0 && this.JumpToNode ==null) {
					Paras ps = new Paras();
					ps.SQL = "SELECT ReturnNode,Returner,ReturnerName,IsBackTracking FROM WF_ReturnWork WHERE WorkID="
							+ dbStr + "WorkID  ORDER BY RDT DESC";
					ps.Add(ReturnWorkAttr.WorkID, this.getWorkID());
					DataTable mydt11 = DBAccess.RunSQLReturnTable(ps);
					if (mydt11.Rows.size() == 0) {
						throw new RuntimeException(bp.wf.Glo.multilingual("@没有找到退回流程的记录.", "WorkNode",
								"not_found_my_expected_data", new String[0]));
					}
					...
					if (myGwl.RetrieveFromDBSources() == 0) {
						throw new RuntimeException(bp.wf.Glo.multilingual("@没有找到自己期望的数据.", "WorkNode",
								"not_found_my_expected_data", new String[0]));
					}
					...
					GenerWorkerLists gwls = new GenerWorkerLists();
					gwls.Retrieve(GenerWorkerListAttr.WorkID, this.getHisGenerWorkFlow().getWorkID(),
							GenerWorkerListAttr.FK_Node, this.JumpToNode.getNodeID(), GenerWorkerListAttr.IsPass, 5);
					if (gwls.size() == 0) {
						throw new RuntimeException(bp.wf.Glo.multilingual("@没有找到接收人期望的数据.", "WorkNode",
								"not_found_receiver_expected_data", new String[0]));
					}
					...
					// 增加天数. 考虑到了节假日.
					dtOfShould = Glo.AddDayHoursSpan(new Date(), this.getHisNode().getTimeLimit(),
							this.getHisNode().getTimeLimitHH(), this.getHisNode().getTimeLimitMM(),
							this.getHisNode().getTWay());

					// 应完成日期.
					String sdt = DataType.dateToStr(dtOfShould, DataType.getSysDataTimeFormat());
					...
					para[0] = gwl.getFK_Emp();
					para[1] = gwl.getFK_EmpText();
					String str = bp.wf.Glo.multilingual("@当前工作已经发送给退回人({0},{1}).", "WorkNode",
							"current_work_send_to_returner", para);
					...
					// 写入track.
					this.AddToTrack(ActionType.Forward, this.JumpToEmp, gwl.getFK_Emp(), this.JumpToNode.getNodeID(),
							this.JumpToNode.getName(), bp.wf.Glo.multilingual("退回后发送", "WorkNode", "send_error_2"));
				...
				/// 当前节点是分流节点但是是子线程退回的节点

				/* 检查该退回是否是原路返回 ? */
				ps = new Paras();
				ps.SQL = "SELECT ReturnNode,Returner,ReturnerName,IsBackTracking FROM WF_ReturnWork WHERE WorkID="
						+ dbStr + "WorkID AND ReturnToNode=" + this.getHisGenerWorkFlow().getFK_Node() +"  ORDER BY RDT DESC";
				ps.Add(ReturnWorkAttr.WorkID, this.getWorkID());
				DataTable mydt = DBAccess.RunSQLReturnTable(ps);
				if (mydt.Rows.size() != 0 && ((mydt.Rows.get(0).getValue("IsBackTracking").toString().equals("1") == true&& this.JumpToNode ==null) || isReturnNode==true)) {
					// 有可能查询出来多个，因为按时间排序了，只取出最后一次退回的，看看是否有退回并原路返回的信息。
			...
			// 检查完成条件。
			if (jumpToNode != null && this.getHisNode().getIsEndNode() == true) {
			...
			/// 2019-09-25 计算未来处理人.
			if (this.getHisNode().getIsStartNode() == true && this.getHisFlow().getIsFullSA() == true) {
				FullSA fa = new FullSA(this);
			}

			/// 计算未来处理人.

			/// 2019-09-25 计算业务字段存储到 wf_generworkflow atpara字段里，用于显示待办信息.
			if (this.getHisNode().getIsStartNode()
					&& DataType.IsNullOrEmpty(this.getHisFlow().getBuessFields()) == false) {
				// 存储到表里atPara @BuessFields=电话^Tel^18992323232;地址^Addr^山东济南;
				String[] expFields = this.getHisFlow().getBuessFields().split("[,]", -1);
				String exp = "";
				Attrs attrs = this.rptGe.getEnMap().getAttrs();
			...
			/// 计算业务字段存储到 wf_generworkflow atpara字段里，用于显示待办信息.

			/// 第二步: 进入核心的流程运转计算区域. 5*5 的方式处理不同的发送情况.
			// 执行节点向下发送的25种情况的判断.
			...
			if (this.getIsStopFlow() == false) {
				...
				if (town != null && town.getHisNode().getHisBatchRole() == BatchRole.Group) {
					this.getHisGenerWorkFlow().setIsCanBatch(true);
					this.getHisGenerWorkFlow().Update();
				}
			}
			...
			if (this.getHisNode().getHisPrintDocEnable() == true && this.getHisNode().getBillTemplates().size() > 0) {
				WorkNodePlus.GenerRtfBillTemplate(this);
			}
			...
			/// 触发下一个节点的自动发送, 处理国机的需求. （去掉:2019-05-05）
			...
			/// 判断当前处理人员，可否处理下一步工作.
			if (this.getIsStopFlow() == false && this.town != null && this.HisRememberMe != null
					&& this.HisRememberMe.getEmps().contains("@" + WebUser.getNo() + "@") == true) {
				String url = "MyFlow.htm?FK_Flow=" + this.getHisFlow().getNo() + "&WorkID=" + this.getWorkID()
						+ "&FK_Node=" + town.getHisNode().getNodeID() + "&FID=" + this.rptGe.getFID();
		...
		for (SubFlowAuto sub : subs.ToJavaList()) {
			...
			if (sub.getHisSubFlowModel() == SubFlowModel.SubLevel) {
				...
				if (sub.getSendModel() == 0) {
					...
					bp.wf.Dev2Interface.Flow_ReSetFlowTitle(sub.getSubFlowNo(),
							Integer.parseInt(sub.getSubFlowNo() + "01"), subWorkID);
					...
					for (String val : nodes) {
						...
						if (gwfSub.getWFSta() != WFSta.Complete) {
							...
							if (subNode.getHisRunModel() == RunModel.SubThread) {
								sql = "SELECT COUNT(*) as Num FROM WF_GenerWorkerList WHERE FID=" + gwfSub.getWorkID()
										+ " AND FK_Flow='" + flowNode[0] + "' AND FK_Node="
										+ Integer.parseInt(flowNode[1]) + " AND IsEnable=1 AND IsPass=1";
							} else {
								sql = "SELECT COUNT(*) as Num FROM WF_GenerWorkerList WHERE WorkID="
	...
	private void Deal_Event() throws Exception {
		...
		/// 处理节点到达事件..
		// 执行发送到达事件.
		if (this.getIsStopFlow() == false && this.town != null) {
	...
	private void WhenTranscactionRollbackError(RuntimeException ex) throws Exception {
		...
		try {
			...
			String maxDT = DBAccess.RunSQLReturnStringIsNull("Select Max(RDT) FROM ND"
					+ Integer.parseInt(this.getHisFlow().getNo()) + "Track WHERE WorkID=" + this.getWorkID(), null);
			if (maxDT != null) {
				// 删除发生的日志.
				DBAccess.RunSQL("DELETE FROM ND" + Integer.parseInt(this.getHisFlow().getNo()) + "Track WHERE WorkID="
						...
						+ ActionType.Forward.getValue() + " AND RDT='" + maxDT + "'");
			}
	...
	private void InitStartWorkDataV2() throws NumberFormatException, Exception {
		...
		if (SystemConfig.getIsBSsystem()) {
			...
			if (DataType.IsNullOrEmpty(fk_nodeFrom) == false) {
				...
				bp.wf.Dev2Interface.WriteTrack(this.getHisFlow().getNo(), this.getHisNode().getNodeID(),
						...
						"@PWorkID=" + PWorkID + "@PFlowNo=" + ndFrom.getHisFlow().getNo(), st2, null,null);
			}
		}
			...
			if (this.getHisFlow().getTitleRole().equals("@OutPara")
				...
				if (DataType.IsNullOrEmpty(this.getHisGenerWorkFlow().getTitle())) {
					this.getHisGenerWorkFlow().setTitle(bp.wf.Glo.multilingual("{0},{1}在{2}发起", "WorkNode",
							"start_wf_title", this.getExecer(), this.getExecerName(), DataType.getCurrentDataTimeCN()));
				}
				// throw new
		...
		this.getHisGenerWorkFlow().setRDT(DataType.getCurrentDataTimess()); // this.HisWork.RDT;
		...
		this.getHisGenerWorkFlow().setFK_Dept(this.getHisWork().getRecOfEmp().getFK_Dept());
		this.getHisGenerWorkFlow().setDeptName(this.getHisWork().getRecOfEmp().getFK_DeptText());
		...
		if (this.getHisFlow().getSDTOfFlowRole() == SDTOfFlowRole.BySpecDateField) {
			try {
				...
				this.getHisGenerWorkFlow().setRDTOfSetting(this.getHisWork()
						.GetValStrByKey(this.getHisFlow().GetParaString("SDTOfFlowRole_StartDateField")));
			} catch (RuntimeException ex) {
				String err1 = bp.wf.Glo.multilingual(
						"可能是流程设计错误,获取开始节点[" + this.getHisGenerWorkFlow().getTitle()
								+ "]的整体流程应完成时间有错误,是否包含SysSDTOfFlow字段? 异常信息:{0}.",
		...
		if (this.getHisFlow().getSDTOfFlowRole() == SDTOfFlowRole.ByAllNodes) {
			...
			this.getHisGenerWorkFlow().setSDTOfFlow(DateUtils.format(sdtOfFlow, DataType.getSysDataTimeFormat()));
		...
		if (this.getHisFlow().getSDTOfFlowRole() == SDTOfFlowRole.ByDays) {
			...
			this.getHisGenerWorkFlow()
					.setSDTOfFlow(DateUtils.format(DateUtils.addDay(new Date(), day), DataType.getSysDataTimeFormat()));
		}
		// 加入两个参数. 2013-02-17
		...
		this.rptGe.setFlowStartRDT(DataType.getCurrentDataTimess());
	...
	public final void DoCopyCurrentWorkDataToRpt() throws Exception {
		...
		rptGe.setFlowEnderRDT(DataType.getCurrentDataTimess());
	...
	private void CheckCompleteCondition_IntCompleteEmps() throws Exception {
		...
		for (DataRow dr : dt.Rows) {
			if (emps.contains("@" + dr.getValue(0).toString() + "@")
					|| emps.contains("@" + dr.getValue(0).toString() + "," + dr.getValue(1).toString() + "@")) {
				continue;
			}
	...
	private void NodeSend_53_UnSameSheet_To_HeLiu(Node nd) throws Exception {
		...
		if (nd.getHisFormType() == NodeFormType.RefOneFrmTree
				&& nd.getNodeFrmID().equals(this.getHisNode().getNodeFrmID()) == true) {
			isCopyData = false;
		}
		...
		/// 复制主表数据. edit 2014-11-20 向合流点汇总数据.
		// 复制当前节点表单数据.
		heLiuWK.setFID(0);
	...
	public final TodolistModel getTodolistModel() throws Exception {
		...
		/**
		 * if (this.HisGenerWorkFlow.TransferCustomType ==
		 * TransferCustomType.ByCCBPMDefine)
		 */
