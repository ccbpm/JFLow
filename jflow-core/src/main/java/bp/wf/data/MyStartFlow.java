...
public class MyStartFlow extends Entity {
	...
	public Map getEnMap() throws Exception {
		...
		try {
			AttrOfSearch search = new AttrOfSearch(MyStartFlowAttr.Starter, "发起人", MyStartFlowAttr.Starter, "=",
					WebUser.getNo(), 0, true);
			map.getAttrsOfSearch().Add(search);

			search = new AttrOfSearch(MyStartFlowAttr.WFState, "流程状态", MyStartFlowAttr.WFState, "not in", "('0')", 0,
					true);
			map.getAttrsOfSearch().Add(search);
		} catch (Exception e) {
	...
	public final String DoPrintFrm() throws Exception {
		return "../../WorkOpt/Packup.htm?FileType=zip,pdf&WorkID=" + this.getWorkID() + "&FK_Flow=" + this.getFK_Flow()
				+ "&NodeID=" + this.getFK_Node() + "&FK_Node=" + this.getFK_Node();
	}

	/// 执行诊断
	public final String DoTrack() throws Exception {
		return bp.wf.Glo.getCCFlowAppPath()+"/WF/WFRpt.htm?CurrTab=Truck&WorkID=" + this.getWorkID() + "&FID=" + this.getFID() + "&FK_Flow="
				+ this.getFK_Flow() + "&FK_Node=" + this.getFK_Node();
	}
	...
	public final String DoOpenLastForm() throws Exception {
		Paras pss = new Paras();
		pss.SQL = "SELECT MYPK FROM ND" + Integer.parseInt(this.getFK_Flow()) + "Track WHERE ActionType="
		...
		return bp.wf.Glo.getCCFlowAppPath()+"/WF/CCForm/FrmGener.htm?WorkID=" + this.getWorkID() + "&FK_Flow=" + this.getFK_Flow() + "&FK_MapData="
				+ nd.getNodeFrmID() + "&ReadOnly=1&IsEdit=0";
	}
