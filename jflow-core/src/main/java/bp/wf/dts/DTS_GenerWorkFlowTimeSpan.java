...
public class DTS_GenerWorkFlowTimeSpan extends Method
{
	...
	*/
	...
	public Object Do() throws Exception {
		...
		//设置为上周.
		sql = "UPDATE WF_GenerWorkFlow SET TSpan=" + TSpan.NextWeek.getValue() + " WHERE RDT >= '" + DataType.SysDateFormat(dtBegin)+ " 00:00' AND RDT <= '" + DataType.SysDateFormat(dtEnd) + " 00:00'";
		DBAccess.RunSQL(sql);
		...
		//把上周的，设置为两个周以前.
		sql = "UPDATE WF_GenerWorkFlow SET TSpan=" + TSpan.TowWeekAgo.getValue() + " WHERE RDT >= '" + DataType.SysDateFormat(dtBegin) + " 00:00' AND RDT <= '" + DataType.SysDateFormat(dtEnd) + " 00:00' ";
		DBAccess.RunSQL(sql);

		//把上周的，设置为更早.
		sql = "UPDATE WF_GenerWorkFlow SET TSpan=" + TSpan.More.getValue() + " WHERE RDT <= '" + DataType.SysDateFormat(dtBegin) + " 00:00' ";
		DBAccess.RunSQL(sql);
