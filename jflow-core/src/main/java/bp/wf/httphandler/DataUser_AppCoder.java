...
public class DataUser_AppCoder extends WebContralBase {
	...
	public final String FlowDesignerWelcome_Init() throws Exception {
		...
		if (bp.wf.Glo.getCCBPMRunModel() == CCBPMRunModel.GroupInc) {
			ht.put("NodeNum", -1); // 节点数据
		} else {
			ht.put("NodeNum", DBAccess.RunSQLReturnValInt("SELECT COUNT(NodeID) FROM WF_Node " + whereStr)); // 节点数据
		}

		// 表单数.
		ht.put("FromNum", DBAccess.RunSQLReturnValInt("SELECT COUNT(No) FROM Sys_MapData  WHERE FK_FormTree !=''"
				+ whereStrPuls + " AND FK_FormTree IS NOT NULL ")); // 表单数

		// 所有的实例数量.
		ht.put("FlowInstaceNum", DBAccess
				.RunSQLReturnValInt("SELECT COUNT(WorkID) FROM WF_GenerWorkFlow WHERE WFState >1 " + whereStrPuls)); // 实例数.

		// 所有的待办数量.
		ht.put("TodolistNum", DBAccess
				.RunSQLReturnValInt("SELECT COUNT(WorkID) FROM WF_GenerWorkFlow WHERE WFState=2 " + whereStrPuls));

		// 退回数.
		ht.put("ReturnNum", DBAccess
				.RunSQLReturnValInt("SELECT COUNT(WorkID) FROM WF_GenerWorkFlow WHERE WFState=5 " + whereStrPuls));

		// 说有逾期的数量. 应该根据 WF_GenerWorkerList的 SDT 字段来求.
		if (SystemConfig.getAppCenterDBType() == DBType.MySQL) {
			ht.put("OverTimeNum", DBAccess.RunSQLReturnValInt(
					"SELECT COUNT(*) FROM WF_GenerWorkerList WHERE IsPass=0 AND STR_TO_DATE(SDT,'%Y-%m-%d %H:%i') < now()"));
			...
			ht.put("OverTimeNum", DBAccess.RunSQLReturnValInt(
					"SELECT COUNT(*) FROM WF_GenerWorkerList WHERE IsPass=0 AND convert(varchar(100),SDT,120) < CONVERT(varchar(100), GETDATE(), 120)"));
		}
	...
	public final String FlowDesignerWelcome_DataSet() throws Exception {
		...
		/// 实例分析
		// 月份分组.
		String sql = "SELECT FK_NY, count(WorkID) as Num FROM WF_GenerWorkFlow WHERE WFState >1 " + whereStrPuls
				+ " GROUP BY FK_NY ";
		...
		// 部门分组.
		sql = "SELECT DeptName, count(WorkID) as Num FROM WF_GenerWorkFlow WHERE WFState >1 " + whereStrPuls
				+ "  GROUP BY DeptName ";
		...
		if (bp.wf.Glo.getCCBPMRunModel() == CCBPMRunModel.Single) {
			...
			sql = "SELECT B.Name, count(a.WorkID) as Num FROM WF_GenerWorkerList A,Port_Dept B WHERE A.FK_Dept=B.No AND B.OrgNo='"
					+ WebUser.getOrgNo() + "' GROUP BY B.Name";
		}
		...
		if (bp.wf.Glo.getCCBPMRunModel() == CCBPMRunModel.Single) {
			...
			sql = "SELECT c.FlowName, count(a.WorkID) as Num FROM WF_GenerWorkerList A,Port_Dept B, WF_GenerWorkFlow C WHERE A.FK_Dept=B.No AND A.WorkID=C.WorkID AND A.IsPass=0 AND C.OrgNo='"
					+ WebUser.getOrgNo() + "' GROUP BY C.FlowName";
		}

		// sql = "SELECT FlowName as name, count(WorkID) as value FROM WF_EmpWorks WHERE
		// WFState >1 GROUP BY FlowName";
		...
		// sql = "SELECT DeptName, count(WorkID) as Num FROM WF_EmpWorks WHERE WFState
		// >1 GROUP BY DeptName";
