...
public class WF_Admin_AttrFlow extends WebContralBase
{
	...
	*/
	public final String APICodeFEE_Init() throws Exception
	{
		...
		String code = bp.da.DataType.ReadTextFile(tmpPath); //, System.Text.Encoding.UTF8).replace("F001Templepte", string.Format("FEE{0}", flow.getNo())).replace("@FlowName", flow.getName()).replace("@FlowNo", flow.getNo());
	...
	*/
	...
	public final String DTSBTable_Init() throws Exception
	{
		...
		if (src.getDBSrcType() == DBSrcType.Oracle || src.getDBSrcType() == DBSrcType.PostgreSQL)
		{
	...
	*/
	public final String DTSBTable_Save() throws Exception
	{
		...
		Flow flow = new Flow(this.getFK_Flow());
		//获取主键方式
		bp.wf.template.DataDTSWay dtsWay = bp.wf.template.DataDTSWay.forValue(this.GetRequestValInt("RB_DTSWay"));
	...
	public final String DTSBTableExt_Init() throws Exception
	{
		...
		if (src.getDBSrcType() == DBSrcType.Oracle || src.getDBSrcType() == DBSrcType.PostgreSQL)
		{
			dtColms.Columns.get("NO").setColumnName("No");
			dtColms.Columns.get("NAME").setColumnName("Name");
		}
	...
	public final String StartGuide_Save() throws Exception
	{
		try
		{
			...
			if (en.getStartGuideWay() == StartGuideWay.BySQLMulti)
			{
				...
				en.setStartGuideWay(StartGuideWay.BySQLMulti);
			}
			//多条-子父流程-合卷审批.
	...
	*/
	public final String TruckViewPower_Save()
	{
		try
		{
			...
			Object tempVar = PubClass.CopyFromRequestByPost(en);
	...
	*/
	public final String PushMsgEntity_Init() throws Exception
	{
		...
		attrs.Retrieve(MapAttrAttr.FK_MapData, "ND" + Integer.parseInt(this.getFK_Flow()) + "rpt", "LGType", 0, "MyDataType", 1);
