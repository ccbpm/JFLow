...
public class WF_Admin_CCBPMDesigner extends WebContralBase
{
	...
	*/
	public final String SelectFlows_Init() throws Exception
	{
		...
		{
			sql = "SELECT CONCAT('F', No) No, Name, CONCAT('F', ParentNo) ParentNo  FROM WF_FlowSort WHERE No='" + fk_flowsort + "' OR ParentNo='" + fk_flowsort + "' ORDER BY Idx";
		}
		...
		if (SystemConfig.getAppCenterDBType() == DBType.Oracle || SystemConfig.getAppCenterDBType() == DBType.PostgreSQL)
			sql = "SELECT No as No, No||'.'||Name as Name,'F'||FK_FlowSort as ParentNo, Idx FROM WF_Flow Where FK_FlowSort='"+fk_flowsort+"'";

		if (SystemConfig.getAppCenterDBType() == DBType.MySQL)
			sql = "SELECT No,CONCAT(CONCAT(No,'.'), Name)  Name, CONCAT('F', FK_FlowSort) ParentNo, Idx FROM WF_Flow Where FK_FlowSort='"+fk_flowsort+"'";
	...
	*/
	public final String Designer_SaveNodeName() throws Exception
	{
		...
		sql = "UPDATE Sys_MapData SET Name='" + this.getName() + "' WHERE No='" + frmID + "'  AND ( Name='' OR Name IS Null) ";
		DBAccess.RunSQL(sql);
	   ...
	   // BP.WF.Template.Cond
		...
		if (groups.size() == 1)
		{
			bp.sys.GroupField group = groups.get(0) instanceof bp.sys.GroupField ? (bp.sys.GroupField)groups.get(0) : null;
	...
	public final String Designer_Save() throws Exception
	{
		...
		try
		{
			...
			for (String str : nodes)
			{
				...
				sBuilder.append("UPDATE WF_Node SET X=" + strss[1] + ",Y=" + strss[2] + ",Name='" + strss[3] + "' WHERE NodeID=" + strss[0] + ";");
			}
	...
	*/
	public final String SelectOneOrg_Selected() throws Exception
	{
		...
		return "url@Default.htm?SID=" + WebUser.getSID() + "&UserNo=" + WebUser.getNo() + "&OrgNo=" + WebUser.getOrgNo();
		// return "登录成功.";
	}
	...
	*/
	public final String GetFlowTreeTable() throws Exception
	{
		...
		String sql = "SELECT * FROM (SELECT 'F'+No as NO,'F'+ParentNo PARENTNO, NAME, IDX, 1 ISPARENT,'FLOWTYPE' TTYPE, -1 DTYPE FROM WF_FlowSort  " + "union " + "SELECT NO, 'F'+FK_FlowSort as PARENTNO,(NO + '.' + NAME) as NAME,IDX,0 ISPARENT,'FLOW' TTYPE, 0 as DTYPE FROM WF_Flow ) A  ORDER BY DTYPE, IDX,NO ";

		if (SystemConfig.getAppCenterDBType() == DBType.Oracle || SystemConfig.getAppCenterDBType() == DBType.PostgreSQL)
		{
			sql = "SELECT * FROM (SELECT 'F'||No as NO,'F'||ParentNo as PARENTNO,NAME, IDX, 1 ISPARENT,'FLOWTYPE' TTYPE,-1 DTYPE FROM WF_FlowSort " + "  union " + "SELECT NO, 'F'||FK_FlowSort as PARENTNO,NO||'.'||NAME as NAME,IDX,0 ISPARENT,'FLOW' TTYPE,0 as DTYPE FROM WF_Flow ) A  ORDER BY DTYPE, IDX,NO";
		}

		if (SystemConfig.getAppCenterDBType() == DBType.MySQL)
		{
			sql = "SELECT * FROM (SELECT CONCAT('F', No) NO, CONCAT('F', ParentNo) PARENTNO, NAME, IDX, 1 ISPARENT,'FLOWTYPE' TTYPE,-1 DTYPE FROM WF_FlowSort " + "  " + "union " + "SELECT NO, CONCAT('F', FK_FlowSort) PARENTNO, CONCAT(NO, '.', NAME) NAME,IDX,0 ISPARENT,'FLOW' TTYPE, 0 as DTYPE FROM WF_Flow " + " ) A  ORDER BY DTYPE, IDX,NO";
		}
	...
	public final String GetFlowTreeTable_GroupInc() throws Exception
	{
		...
		if (SystemConfig.getAppCenterDBType() == DBType.Oracle || SystemConfig.getAppCenterDBType() == DBType.PostgreSQL)
		{
			sql = "SELECT * FROM (SELECT 'F'||No as NO,'F'||ParentNo as PARENTNO,NAME, IDX, 1 ISPARENT,'FLOWTYPE' TTYPE,-1 DTYPE FROM WF_FlowSort " + " WHERE OrgNo ='" + WebUser.getOrgNo() + "' or No = 1 union " + "SELECT NO, 'F'||FK_FlowSort as PARENTNO,NO||'.'||NAME as NAME,IDX,0 ISPARENT,'FLOW' TTYPE,0 as DTYPE FROM WF_Flow WHERE OrgNo ='" + WebUser.getOrgNo() + "') A  ORDER BY DTYPE, IDX";
		}

		if (SystemConfig.getAppCenterDBType() == DBType.MySQL)
		{
			sql = "SELECT * FROM (SELECT CONCAT('F', No) NO, CONCAT('F', ParentNo) PARENTNO, NAME, IDX, 1 ISPARENT,'FLOWTYPE' TTYPE,-1 DTYPE FROM WF_FlowSort " + " WHERE OrgNo ='" + WebUser.getOrgNo() + "' or No = 1 " + "union " + "SELECT NO, CONCAT('F', FK_FlowSort) PARENTNO, CONCAT(NO, '.', NAME) NAME,IDX,0 ISPARENT,'FLOW' TTYPE, 0 as DTYPE FROM WF_Flow " + " WHERE OrgNo ='" + WebUser.getOrgNo() + "') A  ORDER BY DTYPE, IDX";
		}
	...
	public final String GetFormTreeTable_GroupInc() throws Exception
	{
		...
		if (SystemConfig.getAppCenterDBType() == DBType.Oracle)
		{
			sqls = "SELECT No \"No\", ParentNo \"ParentNo\",Name \"Name\", Idx \"Idx\", 1 \"IsParent\", 'FORMTYPE' \"TType\" FROM Sys_FormTree WHERE OrgNo ='" + WebUser.getOrgNo() + "'  or No = 1  ORDER BY Idx ASC ; ";
			sqls += "SELECT No \"No\", FK_FormTree as \"ParentNo\", Name \"Name\",Idx \"Idx\", 0 \"IsParent\", 'FORM' \"TType\" FROM Sys_MapData  WHERE OrgNo ='" + WebUser.getOrgNo() + "' AND AppType=0 AND FK_FormTree IN (SELECT No FROM Sys_FormTree) ORDER BY Idx ASC";
		}
		else
		{
			sqls = "SELECT No,ParentNo,Name, Idx, 1 IsParent, 'FORMTYPE' TType FROM Sys_FormTree WHERE OrgNo ='" + WebUser.getOrgNo() + "'  OR No = 1  ORDER BY Idx ASC ; ";
			sqls += "SELECT No, FK_FormTree as ParentNo,Name,Idx,0 IsParent, 'FORM' TType FROM Sys_MapData  WHERE OrgNo ='" + WebUser.getOrgNo() + "' AND AppType=0 AND FK_FormTree IN (SELECT No FROM Sys_FormTree) ORDER BY Idx ASC";
		}
