...
public class WF_MyFlow extends WebContralBase
{
	...
	public final String MyFlow_Init() throws Exception
	{
		...
		if (this.getCurrND().getIsOpenOver() == true)
		{
			...
			if (this.getCurrND().getIsStartNode() == false)
			{
				...
				if (this.getCurrND().getFrmWorkCheckSta() == FrmWorkCheckSta.Enable)
				{
					...
					bp.wf.Dev2Interface.WriteTrackWorkCheck(gwf.getFK_Flow(), this.getCurrND().getNodeID(), gwf.getWorkID(), gwf.getFID(), msg, workCheck.getFWCOpLabel());
				}
		...
		if (this.getCurrND().getHisFormType() == NodeFormType.SheetTree || this.getCurrND().getHisFormType() == NodeFormType.SheetAutoTree)
		{
			...
			if (this.getIsMobile() == true)
			{
				...
				{
					toUrl = "MyFlowGener.htm?WorkID=" + this.getWorkID() + "&FK_Flow=" + this.getFK_Flow() + "&UserNo=" + WebUser.getNo() + "&FID=" + this.getFID() + "&SID=" + WebUser.getSID() + "&PFlowNo=" + gwf.getPFlowNo() + "&PNodeID=" + gwf.getPNodeID() + "&PWorkID=" + gwf.getPWorkID() + "&Frms=" + gwf.getParasFrms();
				}
				else
				{
					toUrl = "MyFlowGener.htm?WorkID=" + this.getWorkID() + "&FK_Flow=" + this.getFK_Flow() + "&UserNo=" + WebUser.getNo() + "&FID=" + this.getFID() + "&SID=" + WebUser.getSID() + "&PFlowNo=" + gwf.getPFlowNo() + "&PNodeID=" + gwf.getPNodeID() + "&PWorkID=" + gwf.getPWorkID();
				}
			}
			else
			{
				if (gwf.getParasFrms().equals("") == false)
				{
					toUrl = "MyFlowTree.htm?WorkID=" + this.getWorkID() + "&FK_Flow=" + this.getFK_Flow() + "&UserNo=" + WebUser.getNo() + "&FID=" + this.getFID() + "&SID=" + WebUser.getSID() + "&PFlowNo=" + gwf.getPFlowNo() + "&PNodeID=" + gwf.getPNodeID() + "&PWorkID=" + gwf.getPWorkID() + "&Frms=" + gwf.getParasFrms();
				}
				else
				{
					toUrl = "MyFlowTree.htm?WorkID=" + this.getWorkID() + "&FK_Flow=" + this.getFK_Flow() + "&UserNo=" + WebUser.getNo() + "&FID=" + this.getFID() + "&SID=" + WebUser.getSID() + "&PFlowNo=" + gwf.getPFlowNo() + "&PNodeID=" + gwf.getPNodeID() + "&PWorkID=" + gwf.getPWorkID();
				}
			}
		...
		try
		{
			...
			if (this.getIsCC())
			{
				...
				if (fwc.getHisFrmWorkCheckSta() != FrmWorkCheckSta.Enable)
				{
					...
					//toolbar += "<input type=button  value='填写审核意见' enable=true onclick=\"WinOpen('" + appPath + "WF/WorkOpt/CCCheckNote.htm?WorkID=" + this.WorkID + "&FK_Flow=" + this.FK_Flow + "&FID=" + this.getFID() + "&FK_Node=" + this.FK_Node + "&s=" + tKey + "','ds'); \" />";
				}
				return toolbar;
			}
			...
			if (btnLab.getHuiQianRole() == HuiQianRole.Teamup)
			{
				...
				dr.setValue("Oper", btnLab.getSendJS() + " if(SysCheckFrm()==false) return false;Send(true, " + nd.getFormType().getValue() + ");");
				dt.Rows.add(dr);
			...
			if (this.getCurrND().getHisFormType() != NodeFormType.SelfForm && isShowSend == true)
			{
				...
				if (getCurrND().getIsEndNode())
				{
					...
					if (btnLab.getSendEnable() && getCurrND().getHisBatchRole() != BatchRole.Group)
					{
						...
						dr.setValue("No", "Send");
						dr.setValue("Name", btnLab.getSendLab());
						dr.setValue("Oper", btnLab.getSendJS() + " if(SysCheckFrm()==false) return false;SaveDtlAll();Send(false, " + nd.getFormType().getValue() + ");");
						dt.Rows.add(dr);
			...
			{
				String urlr = appPath + "WF/WorkOpt/PrintDoc.htm?FK_Node=" + this.getFK_Node() + "&FID=" + this.getFID() + "&WorkID=" + this.getWorkID() + "&FK_Flow=" + this.getFK_Flow();
			...
			//增加转向下拉框数据.
			if (nd.getCondModel() == DirCondModel.SendButtonSileSelect && isShowSend==true)
			{
				if (nd.getIsStartNode() == true || (gwf.getTodoEmps().contains(WebUser.getNo() + ",") == true))
				{
					...
					for (bp.wf.template.NodeSimple item : nds.ToJavaList())
					{
						...
						else if (item.getHisDeliveryWay() == DeliveryWay.ByDeptAndEmpField)
						{
							dr.setValue("IsSelectEmps", "4");
						}
						else
						{
	...
	*/
	public final String Send() throws Exception
	{
		try
		{
			...
			objs = bp.wf.Dev2Interface.Node_SendWork(this.getFK_Flow(), this.getWorkID(), ht, null, this.getToNode(), null, WebUser.getNo(), WebUser.getName(), WebUser.getFK_Dept(), WebUser.getFK_DeptName(), null, this.getFID(), this.getPWorkID(),GetRequestValBoolen("IsRturnNode"));
			...
			switch (currNode.getHisTurnToDeal())
			{
				...
				case TurnToByCond:
					...
					//if (tts.size() == 0)
					//{
					//    BP.WF.Dev2Interface.Port_SendMsg("admin", currFlow.Name + "在" + currND.Name + "节点处，出现错误", "您没有设置节点完成后的转向条件。", "Err" + currND.No + "_" + this.WorkID, SMSMsgType.Err, this.FK_Flow, this.FK_Node, this.WorkID, this.getFID());
					...
					//        url += "&PFlowNo=" + this.FK_Flow + "&FromNode=" + this.FK_Node + "&PWorkID=" + this.WorkID + "&UserNo=" + WebUser.getNo() + "&SID=" + WebUser.getSID();
					//        return "url@" + url;
					//    }
					//}
					return msg;
	...
	*/
	public final String Save() throws Exception
	{
		try
		{
			String str = bp.wf.Dev2Interface.Node_SaveWork(this.getFK_Flow(), this.getFK_Node(), this.getWorkID(), this.GetMainTableHT(), null, this.getFID(), this.getPWorkID());
...
///#warning 不再支持工具栏的连接，可以使用表单来完成，实现该功能。
		...
		//    formTree.setNo(item.OID.ToString();
		//    formTree.setParentNo("1";
		//    formTree.setName( item.Title;
		...
		//if (appFlowFormTree.size()==1 && nd.FormType== NodeFormType.Tab)
		//{
		//    //nd.FormType
		//}
	...
	*/
	public final String GenerWorkNode() throws Exception
	{
		...
		try
		{
			ds = bp.wf.CCFlowAPI.GenerWorkNode(this.getFK_Flow(), this.getCurrND(), workID, this.getFID(), WebUser.getNo(),this.getWorkID());
	...
	public String MyFlow_StartThread() throws Exception
	{
		...
		for (GenerWorkFlow gwfSubFlow : gwfs.ToJavaList())
		{
			...
			gwfSubFlow.Update();
			//把草稿移交给当前人员. - 更新工作人员列表.
			DBAccess.RunSQL("UPDATE WF_GenerWorkerList SET FK_Emp='" + WebUser.getNo() + "',FK_EmpText='" + bp.web.WebUser.getName() + "' WHERE WorkID=" + gwfSubFlow.getWorkID());
			//更新track表.
