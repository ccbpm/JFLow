...
public class NodeExt extends Entity
{
	...
	*/
	...
	public Map getEnMap() throws Exception
	{
		...
		map.AddTBDecimal(NodeAttr.PassRate, new BigDecimal(100), "完成通过率", true, false);
		...
		map.AddBoolean(NodeAttr.IsDeleteModel, false, "是否删除中间节点信息(不是原来返回时有效)", true, true, false);
	...
	*/
	...
	{
		return "../../Admin/AttrNode/GenerWorkerListDelRole.htm?s=d34&FK_Flow=" + this.getFK_Flow() + "&FK_Node=" + this.getNodeID();
	}
	/** 
	 多人处理规则.
	...
	*/
	...
	{
		return "../../Admin/AttrNode/TurnTo/0.TurntoDefault.htm?FK_Node=" + this.getNodeID() + "&FK_Flow=" + this.getFK_Flow();
	}
	/** 
	 抄送人规则
	...
	*/
	...
	{
		return "../../Comm/En.htm?EnName=BP.WF.Template.CC&PKVal=" + this.getNodeID() + "&FK_Node=" + this.getNodeID() + "&FK_Flow=" + this.getFK_Flow();
	}
	/** 
	 个性化接受人窗口
