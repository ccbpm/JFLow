...
@Scope("request")
public class S4_RenShiController {

	public PrintWriter out;

	@RequestMapping(value = "/Btn_Send_Click4", method = RequestMethod.POST)
	...
	public void Btn_Save_Click(HttpServletRequest request,HttpServletResponse response) {
		...
		en.setNoteRL(request.getParameter("TB_NoteRL"));
		en.Update(); // 写入部门经理意见.
	}

	@RequestMapping(value = "/Btn_Track_Click4", method = RequestMethod.POST)
	public void Btn_Track_Click(HttpServletRequest request,HttpServletResponse response) {
		// BP.WF.Dev2Interface.UI_Window_OneWork(this.getFK_Flow(),this.getWorkID(),
		// this.getFID());
		try {
			out = response.getWriter();
	...
	@RequestMapping(value = "/Btn_Return_Click4", method = RequestMethod.POST)
