...
function getCellText(cell){
	...
	let cellRange = cell.Range;
	cellRange.SetRange(cellRange.Start , cellRange.End-1);
	var cellRangeText = cellRange.Text.ltrim();
	cellRangeText=cellRangeText.rtrim();
	return cellRangeText;
}
function GetServerTemplateDataJSON(template, dataJSON,childDatas) {
	...
	dataJSON.forEach(function(it) {
			 ...
			 if(it.type === "table"){
				// myRange.Text = value;
				// createTable(it.data,rows,myTable);
				congTable = it;
			 }
	...
	for(let i = 1; i <= tables.Count; i++){
		var myTable = tables.Item(i);
		var data = dataJSON;
		
		
		for(let k = 1; k <= myTable.Rows.Count; k++){
			var rows = myTable.Rows.Item(k);
			var firstCell = rows.Cells.Item(1);
			var firstCellText = getCellText(firstCell);
			if(congTable && firstCellText==congTable.name){//子表
				if(congTable.type === "table"){
					// myRange.Text = value;
					 createTable(congTable.data,rows,myTable);
				}
				
			}else{
				for(let i = 1; i <= rows.Range.Cells.Count; i++){
					var cell = rows.Range.Cells.Item(i);
					mt = getCellText(cell);
	...
	//Application.ActiveDocument.Protect(2,null,"123!@#");//加密
	//Application.ActiveDocument.Unprotect(strPassword);
	
}
/* 主表中的从表 */
function createTable(ChildData,rows,mainTable){
	var data = ChildData;
	//var tables = Application.ActiveDocument.Tables;
...
/* 	tblNew.Rows.Add(tblNew.Rows.Item(3));
	...
	mRange.Paste(); */
 	for(j=0;j<data.length;j++){
		var json = data[j];
		let rowNew = mainTable.Rows.Add(mainTable.Rows.Item(j+rowsIndex+1));
		var mRange = mainTable.Rows.Item(j+rowsIndex+1).Range;
		mRange.Paste();
